...
bool NSCANgClient::loadModuleEx(std::string alias, NSCAPI::moduleLoadMode) {
  try {
    ...
    settings.set_alias("NSCA-NG", std::move(alias), "client");
    client_.set_path(settings.alias().get_settings_path("targets"));

    // clang-format off
    settings.alias().add_path_to_settings()
      ("NSCA-NG CLIENT SECTION", "Section for NSCA-NG passive check module.")
...
void NSCANgClient::add_target(const std::string &key, const std::string &arg) {
  try {
...
void NSCANgClient::add_command(const std::string &key, const std::string &arg) {
  try {
    nscapi::core_helper core(get_core(), get_id());
    std::string k = client_.add_command(key, arg);
    if (!k.empty()) core.register_command(k.c_str(), "NSCA-NG relay for: " + key);
  } catch (const std::exception &e) {
...
bool NSCANgClient::unloadModule() {
  client_.clear();
  return true;
}

void NSCANgClient::query_fallback(const PB::Commands::QueryRequestMessage &request_message, PB::Commands::QueryResponseMessage &response_message) {
  client_.do_query(request_message, response_message);
}

bool NSCANgClient::commandLineExec(int target_mode, const PB::Commands::ExecuteRequestMessage &request, PB::Commands::ExecuteResponseMessage &response) {
...
void NSCANgClient::handleNotification(const std::string &, const PB::Commands::SubmitRequestMessage &request_message,

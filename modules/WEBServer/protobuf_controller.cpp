...
BaseController::BaseController(boost::shared_ptr<session_manager_interface> session, nscapi::core_wrapper* core, unsigned int plugin_id, boost::shared_ptr<client::cli_client> client)
  : session(session)
  , core(core)
  , plugin_id(plugin_id)
  , client(client)
  , status("ok") 
{
	addRoute("GET", "/registry/control/module/load", this, &BaseController::registry_control_module_load);
	addRoute("GET", "/registry/control/module/unload", this, &BaseController::registry_control_module_unload);
	addRoute("GET", "/registry/inventory", this, &BaseController::registry_inventory);
	addRoute("GET", "/registry/inventory/modules", this, &BaseController::registry_inventory_modules);
	addRoute("GET", "/settings/inventory", this, &BaseController::settings_inventory);
	addRoute("POST", "/settings/query.json", this, &BaseController::settings_query_json);
	addRoute("POST", "/query.pb", this, &BaseController::run_query_pb);
	addRoute("POST", "/settings/query.pb", this, &BaseController::settings_query_pb);
	addRoute("GET", "/settings/status", this, &BaseController::settings_status);
	addRoute("GET", "/log/status", this, &BaseController::log_status);
	addRoute("GET", "/log/reset", this, &BaseController::log_reset);
	addRoute("GET", "/log/messages", this, &BaseController::log_messages);
	addRoute("GET", "/auth/token", this, &BaseController::auth_token);
	addRoute("GET", "/auth/logout", this, &BaseController::auth_logout);
	addRoute("POST", "/auth/token", this, &BaseController::auth_token);
	addRoute("POST", "/auth/logout", this, &BaseController::auth_logout);
	addRoute("GET", "/core/reload", this, &BaseController::reload);
	addRoute("GET", "/core/isalive", this, &BaseController::alive);
	addRoute("GET", "/console/exec", this, &BaseController::console_exec);
	addRoute("GET", "/metrics", this, &BaseController::get_metrics);
	addRoute("GET", "/", this, &BaseController::redirect_index);
}

std::string BaseController::get_status() {
  boost::shared_lock<boost::shared_mutex> lock(mutex_, boost::get_system_time() + boost::posix_time::seconds(1));
  if (!lock.owns_lock())
    return "unknown";
  return status;
}
bool BaseController::set_status(std::string status_) {
  boost::shared_lock<boost::shared_mutex> lock(mutex_, boost::get_system_time() + boost::posix_time::seconds(1));
  if (!lock.owns_lock())
    return false;
  status = status_;
...
void BaseController::console_exec(Mongoose::Request &request, Mongoose::StreamResponse &response) {
...
void BaseController::log_status(Mongoose::Request &request, Mongoose::StreamResponse &response) {
  if (!session->is_loggedin(request, response))
    return;

...
void legacy_controller::registry_inventory(Mongoose::Request &request, Mongoose::StreamResponse &response) {
  ...
  if (request.get("all", "true") == "true") payload->mutable_inventory()->set_fetch_all(true);
...
void legacy_controller::settings_inventory(Mongoose::Request &request, Mongoose::StreamResponse &response) {
  ...
  if (request.get("paths", "false") == "true") payload->mutable_inventory()->set_fetch_paths(true);
  if (request.get("keys", "false") == "true") payload->mutable_inventory()->set_fetch_keys(true);
  if (request.get("recursive", "false") == "true") payload->mutable_inventory()->set_recursive_fetch(true);
  if (request.get("samples", "false") == "true") payload->mutable_inventory()->set_fetch_samples(true);
  if (request.get("templates", "false") == "true") payload->mutable_inventory()->set_fetch_templates(true);
  if (request.get("desc", "false") == "true") payload->mutable_inventory()->set_descriptions(true);
  std::string path = request.get("path", "");
  if (!path.empty()) payload->mutable_inventory()->mutable_node()->set_path(path);
  std::string key = request.get("key", "");
  if (!key.empty()) payload->mutable_inventory()->mutable_node()->set_key(key);
  std::string module = request.get("module", "");
  if (!module.empty()) payload->mutable_inventory()->set_plugin(module);

...
std::string decode_key(std::string encoded) { return Mongoose::Helpers::decode_b64(encoded); }
...
bool session_manager_interface::is_loggedin(std::string grant, Mongoose::Request &request, Mongoose::StreamResponse &response) {
  ...
  std::string token = request.readHeader("TOKEN");
  if (token.empty()) token = request.get("__TOKEN", "");
  if (!token.empty()) {
    if (!tokens.validate(token)) {
      response.setCodeForbidden("403 You're not allowed");
      return false;
...
bool session_manager_interface::has_user(std::string user) const { return users.find(user) != users.end(); }

void session_manager_interface::add_grant(std::string role, std::string grant) { tokens.add_grant(role, grant); }
...
void session_manager_interface::add_log_message(bool is_error, error_handler_interface::log_entry entry) { log_data->add_message(is_error, entry); }
error_handler_interface *session_manager_interface::get_log_data() { return log_data; }
void session_manager_interface::reset_log() { log_data->reset(); }

void session_manager_interface::set_allowed_hosts(std::string host) { allowed_hosts.set_source(host); }
void session_manager_interface::set_allowed_hosts_cache(bool value) { allowed_hosts.cached = value; }
...
bool session_manager_interface::validate_token(std::string token) { return tokens.validate(token); }

void session_manager_interface::revoke_token(std::string token) { tokens.revoke(token); }

std::string session_manager_interface::generate_token(std::string user) { return tokens.generate(user); }

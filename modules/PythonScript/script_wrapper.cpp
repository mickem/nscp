...
script_wrapper::status script_wrapper::nagios_return_to_py(int code) {
  if (code == NSCAPI::query_return_codes::returnOK) return OK;
  if (code == NSCAPI::query_return_codes::returnWARN) return WARN;
  if (code == NSCAPI::query_return_codes::returnCRIT) return CRIT;
  if (code == NSCAPI::query_return_codes::returnUNKNOWN) return UNKNOWN;
...
int script_wrapper::py_to_nagios_return(status code) {
  ...
  if (code == OK) return NSCAPI::query_return_codes::returnOK;
  if (code == WARN) return NSCAPI::query_return_codes::returnWARN;
  if (code == CRIT) return NSCAPI::query_return_codes::returnCRIT;
  if (code == UNKNOWN) return NSCAPI::query_return_codes::returnUNKNOWN;
...
py::object pystr(std::string str) { return py::object(py::handle<>(PyUnicode_FromString(str.c_str()))); }
...
bool script_wrapper::function_wrapper::has_submit_metrics() { return true; }
...
void script_wrapper::settings_wrapper::set_string(std::string path, std::string key, std::string value) { settings.set_string(path, key, value); }
bool script_wrapper::settings_wrapper::get_bool(std::string path, std::string key, bool def) { return settings.get_bool(path, key, def); }
void script_wrapper::settings_wrapper::set_bool(std::string path, std::string key, bool value) { settings.set_bool(path, key, value); }
int script_wrapper::settings_wrapper::get_int(std::string path, std::string key, int def) { return settings.get_int(path, key, def); }
void script_wrapper::settings_wrapper::set_int(std::string path, std::string key, int value) { settings.set_int(path, key, value); }
boost::python::list script_wrapper::settings_wrapper::get_section(std::string path) { return convert(settings.get_keys(path)); }
void script_wrapper::settings_wrapper::save() { settings.save(); }

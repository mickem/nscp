...
bool PythonScript::commandLineExec(const int target_mode, const PB::Commands::ExecuteRequestMessage::Request &request,
...
void PythonScript::query_fallback(const PB::Commands::QueryRequestMessage::Request &request, PB::Commands::QueryResponseMessage::Response *response,
  ...
  if (inst->has_function(request.command())) {
    ...
    local_response.ParseFromString(buffer);
    if (local_response.payload_size() != 1) return nscapi::protobuf::functions::set_response_bad(*response, "Invalid response: " + request.command());
    response->CopyFrom(local_response.payload(0));
  }
...
void PythonScript::fetchMetrics(PB::Metrics::MetricsMessage::Response *response) {
  ...
  if (inst->has_metrics_fetcher()) {
    ...
    for (const PB::Metrics::MetricsBundle &b : r2.bundles()) response->add_bundles()->CopyFrom(b);
  }
}

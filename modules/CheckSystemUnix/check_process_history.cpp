...
void build_process_history_metrics(PB::Metrics::MetricsBundle *parent, const history_type &data) {
  ...
  for (const process_record &rec : data) {
    ...
    metric(bundle, "times_seen").instance(rec.exe).label("exe", rec.exe).help("Collector samples this process was found in").counter(rec.times_seen);
    metric(bundle, "currently_running")
        .instance(rec.exe)
        .label("exe", rec.exe)
        .help("Whether the process is running right now")
        .gauge(rec.get_currently_running_i());

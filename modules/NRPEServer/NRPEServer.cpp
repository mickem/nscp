...
bool NRPEServer::loadModuleEx(std::string alias, NSCAPI::moduleLoadMode mode) {
  ...
  // The DH parameters ship with the package and are only ever read, so they
  // live in the read-only install tree (${shared-path}/security) rather than in
  // ${certificate-path}, which is the writable directory the generated
  // certificate and CA are placed in.
  socket_helpers::settings_helper::add_ssl_server_opts(settings, info_, true, "${shared-path}/security/nrpe_dh_2048.pem", certificate, "", opts);

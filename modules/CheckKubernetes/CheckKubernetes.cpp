  ...
  try {
    ...
    settings.alias().add_key_to_settings()
      ...
      .add_string("ca", sh::path_key(&defaults_.ca, "${ca-path}"),
        "CERTIFICATE AUTHORITY", "CA bundle (a PEM file or a hashed directory) used to verify the API server certificate. Defaults to the trusted system store; point it at the cluster CA (`kubectl config view --raw -o jsonpath='{.clusters[0].cluster.certificate-authority-data}' | base64 -d`) for a private cluster CA.")
      ...
      .add_int("timeout", sh::int_key(&defaults_.timeout, 30),
        "TIMEOUT", "Timeout for each API server request, in seconds.", true)

...
void CheckEventLog::check_modern(const std::string &logfile, const std::string &scan_range, const int truncate_message, eventlog_filter::filter &filter,
                                 std::string bookmark) {
  typedef eventlog_filter::filter filter_type;
...
void CheckEventLog::CheckEventLog_(PB::Commands::QueryRequestMessage::Request &request, PB::Commands::QueryResponseMessage::Response *response) {
  ...
  if (unique) request.add_arguments("unique");
  if (!filter.empty()) {
...
void CheckEventLog::check_eventlog(const PB::Commands::QueryRequestMessage::Request &request, PB::Commands::QueryResponseMessage::Response *response) {
  ...
  if (scan_range.empty()) scan_range = "-24h";
...
bool CheckEventLog::commandLineExec(const int target_mode, const PB::Commands::ExecuteRequestMessage::Request &request,
                                    PB::Commands::ExecuteResponseMessage::Response *response, const PB::Commands::ExecuteRequestMessage &) {
  std::string command = request.command();
  if (command == "eventlog" && request.arguments_size() > 0) command = request.arguments(0);
...
void CheckEventLog::list_providers(const PB::Commands::ExecuteRequestMessage::Request &request, PB::Commands::ExecuteResponseMessage::Response *response) {
  try {
    ...
    if (!channels && !publishers && !tasks && !keywords && !all) help = true;
    if (help) {
    ...
    if (channels || all) {
      ...
      while (true) {
        ...
        if (channels || all) {
          ...
          bool match = false;
          if (tasks) {
            eventlog::eventlog_table tbl = eventlog::fetch_table(hProvider, eventlog::api::EvtPublisherMetadataTasks,
                                                                 eventlog::api::EvtPublisherMetadataTaskValue, eventlog::api::EvtPublisherMetadataTaskName);
            if (tbl.size() > 0) {
              ss << utf8::cvt<std::string>(buffer.get()) << "\n";
              match = true;
              ss << "Tasks:\n";
          ...
          if (keywords) {
            eventlog::eventlog_table tbl =
                eventlog::fetch_table(hProvider, eventlog::api::EvtPublisherMetadataKeywords, eventlog::api::EvtPublisherMetadataKeywordValue,
                                      eventlog::api::EvtPublisherMetadataKeywordName);
            if (tbl.size() > 0) {
              if (!match) ss << utf8::cvt<std::string>(buffer.get()) << "\n";
              match = true;
              ss << "Keywords:\n";
              for (const eventlog::eventlog_table::value_type &v : tbl) {
                ss << " * " << v.second << "\n";
    ...
    if (publishers || tasks || keywords || all) {
      ...
      if (!publisher.empty()) {
        ...
        if (keywords) {
          eventlog::eventlog_table tbl = eventlog::fetch_table(hProvider, eventlog::api::EvtPublisherMetadataKeywords,
                                                               eventlog::api::EvtPublisherMetadataKeywordValue, eventlog::api::EvtPublisherMetadataKeywordName);
          if (tbl.size() > 0) {
            if (!match) ss << publisher << "\n";
            match = true;
            ss << "Keywords:\n";
            for (const eventlog::eventlog_table::value_type &v : tbl) {
              ss << " * " << v.second << " (" << v.first << ")\n";

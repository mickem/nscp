...
BOOL EvtFormatMessage(api::EVT_HANDLE PublisherMetadata, api::EVT_HANDLE Event, DWORD MessageId, DWORD ValueCount, api::PEVT_VARIANT Values, DWORD Flags,
  ...
  if (!api::pEvtFormatMessage) throw nsclient::nsclient_exception("Failed to load: EvtFormatMessage");
  return api::pEvtFormatMessage(PublisherMetadata, Event, MessageId, ValueCount, Values, Flags, BufferSize, Buffer, BufferUsed);
}
int EvtFormatMessage(api::EVT_HANDLE PublisherMetadata, api::EVT_HANDLE Event, DWORD MessageId, DWORD ValueCount, api::PEVT_VARIANT Values, DWORD Flags,
                     std::string &str) {
  hlp::buffer<wchar_t, LPWSTR> message_buffer(4096);
  DWORD dwBufferSize;
  if (!eventlog::EvtFormatMessage(PublisherMetadata, Event, MessageId, ValueCount, Values, Flags, static_cast<DWORD>(message_buffer.size()),
                                  message_buffer.get(), &dwBufferSize)) {
...
api::EVT_HANDLE EvtCreateBookmark(LPCWSTR BookmarkXml) {
  if (!api::pEvtCreateBookmark) throw nsclient::nsclient_exception("Failed to load: EvtCreateBookmark");
...
api::EVT_HANDLE EvtOpenPublisherMetadata(api::EVT_HANDLE Session, LPCWSTR PublisherId, LPCWSTR LogFilePath, LCID Locale, DWORD Flags) {
  if (!api::pEvtOpenPublisherMetadata) throw nsclient::nsclient_exception("Failed to load: EvtOpenPublisherMetadata");
...
api::EVT_HANDLE EvtCreateRenderContext(DWORD ValuePathsCount, LPCWSTR *ValuePaths, DWORD Flags) {
  if (!api::pEvtCreateRenderContext) throw nsclient::nsclient_exception("Failed to load: EvtCreateRenderContext");
...
BOOL EvtRender(api::EVT_HANDLE Context, api::EVT_HANDLE Fragment, DWORD Flags, DWORD BufferSize, PVOID Buffer, PDWORD BufferUsed, PDWORD PropertyCount) {
  if (!api::pEvtRender) throw nsclient::nsclient_exception("Failed to load: EvtRender");
...
BOOL EvtNext(api::EVT_HANDLE ResultSet, DWORD EventsSize, api::PEVT_HANDLE Events, DWORD Timeout, DWORD Flags, PDWORD Returned) {
  if (!api::pEvtNext) throw nsclient::nsclient_exception("Failed to load: EvtNext");
...
BOOL EvtSeek(api::EVT_HANDLE ResultSet, LONGLONG Position, api::EVT_HANDLE Bookmark, DWORD Timeout, DWORD Flags) {
  if (!api::pEvtSeek) throw nsclient::nsclient_exception("Failed to load: EvtSeek");
...
api::EVT_HANDLE EvtQuery(api::EVT_HANDLE Session, LPCWSTR Path, LPCWSTR Query, DWORD Flags) {
  if (!api::pEvtQuery) throw nsclient::nsclient_exception("Failed to load: EvtQuery");
  return api::pEvtQuery(Session, Path, Query, Flags);
}
api::EVT_HANDLE EvtOpenPublisherEnum(api::EVT_HANDLE Session, DWORD Flags) {
  if (!api::pEvtOpenPublisherEnum) throw nsclient::nsclient_exception("Failed to load: EvtOpenPublisherEnum");
  return api::pEvtOpenPublisherEnum(Session, Flags);
}
BOOL EvtClose(api::EVT_HANDLE Object) {
  if (!api::pEvtClose) throw nsclient::nsclient_exception("Failed to load: EvtClose");
...
BOOL EvtNextPublisherId(api::EVT_HANDLE PublisherEnum, DWORD PublisherIdBufferSize, LPWSTR PublisherIdBuffer, PDWORD PublisherIdBufferUsed) {
  if (!api::ptEvtNextPublisherId) throw nsclient::nsclient_exception("Failed to load: EvtOpenPublisherEnum");
...
api::EVT_HANDLE EvtOpenChannelEnum(api::EVT_HANDLE Session, DWORD Flags) {
  if (!api::pEvtOpenChannelEnum) throw nsclient::nsclient_exception("Failed to load: EvtOpenChannelEnum");
...
BOOL EvtNextChannelPath(api::EVT_HANDLE PublisherEnum, DWORD PublisherIdBufferSize, LPWSTR PublisherIdBuffer, PDWORD PublisherIdBufferUsed) {
  if (!api::pEvtNextChannelPath) throw nsclient::nsclient_exception("Failed to load: EvtNextChannelPath");
...
BOOL EvtGetPublisherMetadataProperty(api::EVT_HANDLE PublisherMetadata, api::EVT_PUBLISHER_METADATA_PROPERTY_ID PropertyId, DWORD Flags,
  ...
  if (!api::pEvtGetPublisherMetadataProperty) throw nsclient::nsclient_exception("Failed to load: EvtGetPublisherMetadataProperty");
  return api::pEvtGetPublisherMetadataProperty(PublisherMetadata, PropertyId, Flags, PublisherMetadataPropertyBufferSize, PublisherMetadataPropertyBuffer,
                                               PublisherMetadataPropertyBufferUsed);
}
BOOL EvtGetObjectArrayProperty(api::EVT_OBJECT_ARRAY_PROPERTY_HANDLE ObjectArray, DWORD PropertyId, DWORD ArrayIndex, DWORD Flags,
                               DWORD PropertyValueBufferSize, api::PEVT_VARIANT PropertyValueBuffer, PDWORD PropertyValueBufferUsed) {
  if (!api::pEvtGetObjectArrayProperty) throw nsclient::nsclient_exception("Failed to load: EvtGetObjectArrayProperty");
...
BOOL EvtGetObjectArraySize(api::EVT_OBJECT_ARRAY_PROPERTY_HANDLE ObjectArray, PDWORD ObjectArraySize) {
  if (!api::pEvtGetObjectArraySize) throw nsclient::nsclient_exception("Failed to load: EvtGetObjectArraySize");
  return api::pEvtGetObjectArraySize(ObjectArray, ObjectArraySize);
}

api::EVT_HANDLE EvtSubscribe(api::EVT_HANDLE Session, HANDLE SignalEvent, LPCWSTR ChannelPath, LPCWSTR Query, api::EVT_HANDLE Bookmark, PVOID context,
                             api::EVT_SUBSCRIBE_CALLBACK Callback, DWORD Flags) {
  if (!api::pEvtSubscribe) throw nsclient::nsclient_exception("Failed to load: EvtSubscribe");
...
long long get_int(const eventlog::api::PEVT_VARIANT &var) {
  ...
  if (var->Type == EvtVarTypeNull) return 0;
  if (var->Type == EvtVarTypeSByte) return var->SByteVal;
  if (var->Type == EvtVarTypeByte) return var->ByteVal;
  if (var->Type == EvtVarTypeInt16) return var->Int16Val;
  if (var->Type == EvtVarTypeUInt16) return var->UInt16Val;
  if (var->Type == EvtVarTypeInt32) return var->Int32Val;
  if (var->Type == EvtVarTypeUInt32) return var->UInt32Val;
  if (var->Type == EvtVarTypeInt64) return var->Int64Val;
  if (var->Type == EvtVarTypeUInt64) return var->UInt64Val;
  if (var->Type == EvtVarTypeHexInt32) return var->Int32Val;
  if (var->Type == EvtVarTypeHexInt64) return var->Int64Val;
  return 0;
}

std::string get_str(const eventlog::api::PEVT_VARIANT &var) {
  using namespace eventlog::api;
  if (var->Type == EvtVarTypeNull) return "";
  if (var->Type == EvtVarTypeAnsiString) return var->AnsiStringVal;
  if (var->Type == EvtVarTypeString) return utf8::cvt<std::string>(var->StringVal);

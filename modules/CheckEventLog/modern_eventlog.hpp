...
typedef EVT_HANDLE(WINAPI* tEvtOpenPublisherEnum)(EVT_HANDLE Session, DWORD Flags);

typedef BOOL(WINAPI* tEvtNextPublisherId)(EVT_HANDLE PublisherEnum, DWORD PublisherIdBufferSize,
...
typedef EVT_HANDLE(WINAPI* tEvtOpenChannelEnum)(_In_ EVT_HANDLE Session, _In_ DWORD Flags);
typedef BOOL(WINAPI* tEvtNextChannelPath)(_In_ EVT_HANDLE ChannelEnum, _In_ DWORD ChannelPathBufferSize, _In_ LPWSTR ChannelPathBuffer,
                                          _Out_ PDWORD ChannelPathBufferUsed);
typedef BOOL(WINAPI* tEvtClose)(_In_ EVT_HANDLE Object);
typedef EVT_HANDLE(WINAPI* tEvtQuery)(EVT_HANDLE Session, LPCWSTR Path, LPCWSTR Query, DWORD Flags);
typedef BOOL(WINAPI* tEvtNext)(EVT_HANDLE ResultSet, DWORD EventsSize, PEVT_HANDLE Events, DWORD Timeout, DWORD Flags, _Out_ PDWORD Returned);

typedef BOOL(WINAPI* tEvtSeek)(EVT_HANDLE ResultSet, LONGLONG Position, EVT_HANDLE Bookmark,
                               _Reserved_ DWORD Timeout,  // currently must be 0
                               DWORD Flags);

typedef EVT_HANDLE(WINAPI* tEvtCreateRenderContext)(DWORD ValuePathsCount, _In_reads_opt_(ValuePathsCount) LPCWSTR* ValuePaths,
                                                    DWORD Flags  // EVT_RENDER_CONTEXT_FLAGS
);

typedef BOOL(WINAPI* tEvtRender)(EVT_HANDLE Context, EVT_HANDLE Fragment,
                                 DWORD Flags,  // EVT_RENDER_FLAGS
                                 DWORD BufferSize, _Out_writes_bytes_to_opt_(BufferSize, *BufferUsed) PVOID Buffer, _Out_ PDWORD BufferUsed,
                                 _Out_ PDWORD PropertyCount);

typedef EVT_HANDLE(WINAPI* tEvtOpenPublisherMetadata)(EVT_HANDLE Session, LPCWSTR PublisherId, LPCWSTR LogFilePath, LCID Locale, DWORD Flags);

typedef BOOL(WINAPI* tEvtFormatMessage)(EVT_HANDLE PublisherMetadata,  // Except for forwarded events
                                        EVT_HANDLE Event, DWORD MessageId, DWORD ValueCount, PEVT_VARIANT Values, DWORD Flags, DWORD BufferSize,
                                        _Out_writes_to_opt_(BufferSize, *BufferUsed) LPWSTR Buffer, _Out_ PDWORD BufferUsed);

typedef BOOL(WINAPI* tEvtGetPublisherMetadataProperty)(EVT_HANDLE PublisherMetadata, EVT_PUBLISHER_METADATA_PROPERTY_ID PropertyId, DWORD Flags,
                                                       DWORD PublisherMetadataPropertyBufferSize, PEVT_VARIANT PublisherMetadataPropertyBuffer,
...
typedef EVT_HANDLE(WINAPI* tEvtSubscribe)(EVT_HANDLE Session, HANDLE SignalEvent, LPCWSTR ChannelPath, LPCWSTR Query, EVT_HANDLE Bookmark, PVOID context,
                                          EVT_SUBSCRIBE_CALLBACK Callback, DWORD Flags);

typedef EVT_HANDLE(WINAPI* tEvtCreateBookmark)(LPCWSTR BookmarkXml);
typedef BOOL(WINAPI* tEvtUpdateBookmark)(EVT_HANDLE Bookmark, EVT_HANDLE Event);
...
struct evt_closer {
  static void close(api::EVT_HANDLE& handle) { EvtClose(handle); }
};

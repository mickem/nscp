...
void do_get(CComPtr<ITaskService> taskSched, tasksched_filter::filter &filter, std::string folder, bool recursive, bool hidden) {
  ...
  if (recursive) {
    ...
    if (FAILED(pRootFolder->GetFolders(0, &folders))) throw nsclient::nsclient_exception("Failed to get folders below " + folder + ": " + error::com::get(hr));
    LONG count = 0;
    if (FAILED(folders->get_Count(&count))) throw nsclient::nsclient_exception("Failed to get count of folders below " + folder + ": " + error::com::get(hr));
    for (LONG i = 0; i < count; ++i) {
      ...
      BSTR str;
      if (FAILED(inst->get_Path(&str))) throw nsclient::nsclient_exception("Failed to get path for " + str::xtos(i) + ": " + error::com::get(hr));

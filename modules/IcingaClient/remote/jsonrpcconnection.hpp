...
//	DECLARE_PTR_TYPEDEFS(JsonRpcConnection);
	typedef JsonRpcConnection* Ptr;

	JsonRpcConnection(const std::string& identity, bool authenticated, const TlsStream::Ptr& stream, ConnectionRole role);
...
// 	static Value HeartbeatAPIHandler(const intrusive_ptr<MessageOrigin>& origin, const Dictionary::Ptr& params);
	...
	static void SendCertificateRequest(const JsonRpcConnection::Ptr& aclient, 
		const MessageOrigin& origin, const std::string& path);
...
private:
	...
	void MessageHandlerWrapper(const std::string& jsonString);
	void MessageHandler(const std::string& jsonString);

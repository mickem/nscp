...
void NSCAClient::setup(client::configuration &config, const ::Plugin::Common_Header& header) {
	...
	BOOST_FOREACH(const std::string &r, strEx::s::splitEx(header.recipient_id(), std::string(","))) {
		if (targets.has_object(r)) {
			nscapi::protobuf::types::destination_container d;
			config.target_lookup->apply(d, r);
			config.data->recipients.push_back(d);
		} else {
			NSC_LOG_ERROR("Target not found: " + r);
		}
	}
	if (config.data->recipients.empty()) {
		nscapi::protobuf::types::destination_container d;
		config.target_lookup->apply(d, "default");
		config.data->recipients.push_back(d);
	}
...
bool NSCAClient::clp_handler_impl::query(client::destination_container sender, client::destination_container target, const Plugin::QueryRequestMessage &request_message, Plugin::QueryResponseMessage &response_message) {
	...
	nscapi::protobuf::functions::make_return_header(response_message.mutable_header(), request_header);
		...
		for (int i=0;i < request_message.payload_size(); ++i) {
			...
			packet.code = 0;
			packet.result = data.command;
			list.push_back(packet);
		}
		boost::tuple<int,std::string> ret = send(target, list);

	nscapi::protobuf::functions::append_simple_query_response_payload(response_message.add_payload(), "TODO",0, "", "");
...
int NSCAClient::clp_handler_impl::submit(client::configuration::data_type data, const Plugin::SubmitRequestMessage &request_message, Plugin::SubmitResponseMessage &response_message) {
	...
	BOOST_FOREACH(NSCAClient::connection_data con, parse_header(request_header, data)) {
	...
	nscapi::protobuf::functions::append_simple_submit_response_payload(response_message.add_payload(), "TODO",0, "");
	return NSCAPI::isSuccess;
}
...
int NSCAClient::clp_handler_impl::exec(client::configuration::data_type data, const Plugin::ExecuteRequestMessage &request_message, Plugin::ExecuteResponseMessage &response_message) {
	...
	nscapi::protobuf::functions::append_simple_exec_response_payload(response_message.add_payload(), "TODO", 0, "");
	return NSCAPI::isSuccess;
}
...
boost::tuple<int, std::string> NSCAClient::clp_handler_impl::send(client::destination_container target, const std::list<nsca::packet> packets) {
	try {

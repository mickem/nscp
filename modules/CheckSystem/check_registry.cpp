...
void registry_key_checks::check(const PB::Commands::QueryRequestMessage::Request &request, PB::Commands::QueryResponseMessage::Response *response) {
  ...
  filter_helper.add_syntax("${status}: ${problem_list}", "${path}: exists=${exists}, subkeys=${subkey_count}, values=${value_count}", "${path}",
                           "${status}: No registry keys found", "${status}: All %(count) registry key(s) are ok.");
...
void registry_value_checks::check(const PB::Commands::QueryRequestMessage::Request &request, PB::Commands::QueryResponseMessage::Response *response) {
  ...
  filter_type filter;
  filter_helper.add_options("", "not exists", "", filter.get_filter_syntax(), "unknown");
  filter_helper.add_syntax("${status}: ${problem_list}", "${path}: ${string_value} (type=${type})", "${path}", "${status}: No registry values found",
                           "${status}: %(list).");

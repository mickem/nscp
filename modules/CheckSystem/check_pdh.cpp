	...
	void counter_config_object::read(boost::shared_ptr<nscapi::settings_proxy> proxy, bool oneliner, bool is_sample) {
		...
		if (oneliner) {
			...
			if (pos != std::string::npos) {
				std::string path = path.substr(0, pos);
				std::string key = path.substr(pos+1);
				proxy->register_key(path, key, NSCAPI::key_string, alias, "Counter " + alias + ". To configure this item add a section called: " + path, "", false, is_sample);

...
load_avg_state state_after(const int ticks, const double queue, const double busy_cores) {
  load_avg_state s;
  for (int i = 0; i < ticks; ++i) s.update(queue, busy_cores);
  s.cores = 4;
  s.procs_total = 1234;
  return s;
}
...
TEST(CheckLoadAvg, ShortWindowReactsFasterOnFallingLoad) {
  load_avg_state s;
  s.update(0.0, 8.0);                           // seed at 8
  for (int i = 0; i < 60; ++i) s.update(0, 0);  // one idle minute
...
TEST(CheckLoadAvg, ShortWindowReactsFasterOnRisingLoad) {
  load_avg_state s;
  s.update(0.0, 0.0);                               // seed idle
  for (int i = 0; i < 60; ++i) s.update(2.0, 6.0);  // one busy minute at 8

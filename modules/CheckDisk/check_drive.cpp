...
std::string type_to_string(const long long type) {
  ...
  if (type == DRIVE_REMOTE) return "remote";
  if (type == DRIVE_RAMDISK) return "ramdisk";
  if (type == DRIVE_UNKNOWN) return "unknown";
  if (type == DRIVE_NO_ROOT_DIR) return "no_root_dir";
  if (type == drive_type_total) return "total";
  return "unknown";
}
...
struct filter_obj {
  ...
  std::string get_flags(parsers::where::evaluation_context) const {
    ...
    if ((drive.flags & drive_container::df_removable) == drive_container::df_removable) str::format::append_list(ret, "removable");
    if ((drive.flags & drive_container::df_readable) == drive_container::df_readable) str::format::append_list(ret, "readable");
    if ((drive.flags & drive_container::df_writable) == drive_container::df_writable) str::format::append_list(ret, "writable");
    if ((drive.flags & drive_container::df_erasable) == drive_container::df_erasable) str::format::append_list(ret, "erasable");
    return ret;
  }
  ...
  long long get_hotplug(parsers::where::evaluation_context context) const { return (drive.flags & drive_container::df_hotplug) == drive_container::df_hotplug; }
  long long get_mounted(parsers::where::evaluation_context context) const { return (drive.flags & drive_container::df_mounted) == drive_container::df_mounted; }
  ...
  long long get_media_type(parsers::where::evaluation_context context) const { return drive.type; }

  std::wstring get_volume_or_letter_w() {
    if (!drive.id.empty()) return utf8::cvt<std::wstring>(drive.id);
    return utf8::cvt<std::wstring>(drive.letter);
  }
  long long get_type(parsers::where::evaluation_context context) {
    if (has_type) return drive_type;
...
int do_convert_type(const std::string &keyword) {
  ...
  if (keyword == "cdrom") return DRIVE_CDROM;
  if (keyword == "removable") return DRIVE_REMOVABLE;
  if (keyword == "remote") return DRIVE_REMOTE;
  if (keyword == "ramdisk") return DRIVE_RAMDISK;
  if (keyword == "unknown") return DRIVE_UNKNOWN;
  if (keyword == "no_root_dir") return DRIVE_NO_ROOT_DIR;
...
class volume_helper {
  ...
  typedef HANDLE(WINAPI *typeFindFirstVolumeMountPointW)(__in LPCWSTR lpszRootPathName, __out_ecount(cchBufferLength) LPWSTR lpszVolumeMountPoint,
                                                         __in DWORD cchBufferLength);
  typedef BOOL(WINAPI *typeFindNextVolumeMountPointW)(__inout HANDLE hFindVolume, __out_ecount(cchBufferLength) LPWSTR lpszVolumeName,
                                                      __in DWORD cchBufferLength);
  typedef BOOL(WINAPI *typeGetVolumeNameForVolumeMountPointW)(__in LPCWSTR lpszVolumeMountPoint, __out_ecount(cchBufferLength) LPWSTR lpszVolumeName,
                                                              __in DWORD cchBufferLength);
  typedef BOOL(WINAPI *typeGetVolumeInformationByHandleW)(_In_ HANDLE hFile, _Out_opt_ LPWSTR lpVolumeNameBuffer, _In_ DWORD nVolumeNameSize,
  ...
  typedef BOOL(WINAPI *typeGetVolumePathNamesForVolumeNameW)(_In_ LPCTSTR lpszVolumeName, _Out_ LPTSTR lpszVolumePathNames, _In_ DWORD cchBufferLength,
 ...
 public:
  ...
  BOOL FindNextVolumeMountPoint(HANDLE hVolume, std::wstring &volume) {
    if (ptrFindNextVolumeMountPointW == NULL || hVolume == INVALID_HANDLE_VALUE) return FALSE;
    hlp::tchar_buffer buffer(1024);
  ...
  void GetVolumeInformationByHandle(HANDLE hVolume, std::wstring &name, std::wstring &fs) {
    if (ptrGetVolumeInformationByHandleW == NULL || hVolume == INVALID_HANDLE_VALUE) return;
    hlp::tchar_buffer volumeName(1024);
    hlp::tchar_buffer fileSysName(1024);
    DWORD maximumComponentLength, fileSystemFlags;

    if (!ptrGetVolumeInformationByHandleW(hVolume, volumeName.get(), static_cast<DWORD>(volumeName.size()), NULL, &maximumComponentLength, &fileSystemFlags,
  ...
  bool getVolumeInformation(std::wstring volume, std::wstring &name, std::wstring &fs, unsigned long long &type, drive_container::drive_flags &flags) {
    ...
    if (hDevice != INVALID_HANDLE_VALUE) {
      ...
      if (DeviceIoControl(hDevice, IOCTL_STORAGE_GET_HOTPLUG_INFO, 0, 0, &Info, sizeof(Info), &ReturnedSize, NULL)) {
        if (Info.MediaRemovable) flags |= drive_container::df_removable;
        if (Info.DeviceHotplug) flags |= drive_container::df_hotplug;
      }
      ...
      if (err == 0 && mediaType.get()->MediaInfoCount > 0) {
        ...
        if (mediaType.get()->DeviceType == FILE_DEVICE_DISK || mediaType.get()->DeviceType == FILE_DEVICE_CD_ROM ||
            mediaType.get()->DeviceType == FILE_DEVICE_DVD) {
          ...
          if (Characteristics & MEDIA_CURRENTLY_MOUNTED) flags |= drive_container::df_mounted;
          if (Characteristics & (MEDIA_READ_ONLY | MEDIA_READ_WRITE)) flags |= drive_container::df_readable;
          if (((Characteristics & MEDIA_READ_WRITE) != 0 || (Characteristics & MEDIA_WRITE_ONCE) != 0) && (Characteristics & MEDIA_WRITE_PROTECTED) == 0 &&
              (Characteristics & MEDIA_READ_ONLY) == 0)
            flags |= drive_container::df_writable;
          if (Characteristics & MEDIA_ERASEABLE) flags |= drive_container::df_erasable;
        }
      }

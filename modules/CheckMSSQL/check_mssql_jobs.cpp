...
// step_id = 0 is the job outcome row (per-step rows use step_id >= 1).
const char *JOBS_SQL =
    "SELECT j.name, j.enabled,"
    " ISNULL(h.run_status, -1) AS run_status,"
    " ISNULL(DATEDIFF(second, msdb.dbo.agent_datetime(h.run_date, h.run_time), GETDATE()), -1) AS last_run_age"
    " FROM msdb.dbo.sysjobs j"
    " OUTER APPLY (SELECT TOP 1 run_status, run_date, run_time FROM msdb.dbo.sysjobhistory h"
...
filter_obj_handler::filter_obj_handler() {
  registry_.add_string_var("name", &filter_obj::get_name, "Job name")
      .add_string_var("last_run_status", &filter_obj::get_last_run_status, "Outcome of the last run: failed, succeeded, retry, canceled, running or never");
  ...
  registry_
      ...
      .no_perf()
      .add_int_var("last_run_outcome", &filter_obj::get_last_run_outcome, "Raw msdb run_status code of the last run (-1 = never ran)")
      .no_perf();
}

...
bool Op5Client::loadModuleEx(std::string alias, NSCAPI::moduleLoadMode mode) {
  try {
    ...
    default_checks["Disk Read Average Latency"] =
        "check_pdh \"counter=\\\\LogicalDisk(*)\\\\Avg. Disk sec/Read\" show-all \"crit=value > 25\" \"warn=value > 20\"";
    default_checks["Disk Write Average Latency"] =
        "check_pdh \"counter=\\\\LogicalDisk(*)\\\\Avg. Disk sec/Write\" show-all \"crit=value > 25\" \"warn=value > 20\"";
...
bool Op5Client::commandLineExec(const int target_mode, const PB::Commands::ExecuteRequestMessage::Request &request,
  ...
  if (command == "op5" && request.arguments_size() > 0)
    ...
    command = "help";
  if (command == "install") return cli_install(request, response);
...
bool Op5Client::cli_install(const PB::Commands::ExecuteRequestMessage::Request &request, PB::Commands::ExecuteResponseMessage::Response *response) {
  ...
  try {
    ...
    result << "Sending status every " << str::format::stox_as_time_sec<unsigned long long>(interval, "s") << " seconds to " << server << " as " << user
           << " identified by " << password << std::endl;
...
void Op5Client::handleNotification(const std::string &, const PB::Commands::SubmitRequestMessage &request_message,

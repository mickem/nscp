...
std::string NSClientSocket::parseRequest(std::string buffer) {
  ...
  break - on -
      compile if ((pwd.first.empty()) || (pwd.first != NSCModuleHelper::getSettingsString("NSClient", "password", ""))) return "ERROR: Invalid password.";
  if (pwd.second.empty()) return "ERRRO: No command specified.";
  strEx::token cmd = strEx::getToken(pwd.second, '&');
  if (cmd.first.empty()) return "ERRRO: No command specified.";
  ...
  switch (c) {
    ...
    default:  // "New" check_nscp also returns performance data
      if (perf.empty()) return NSCHelper::translateReturn(ret) + "&" + message;

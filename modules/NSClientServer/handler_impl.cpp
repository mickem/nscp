...
check_nt::packet handler_impl::handle(check_nt::packet p) {
  ...
  switch (c) {
    ...
    case REQ_CLIENTVERSION: {
      ...
      // if (v == _T("auto"))
      std::wstring v =
          nscapi::plugin_singleton->get_core()->getApplicationName() + _T(" ") + nscapi::plugin_singleton->get_core()->getApplicationVersionString();
  ...
  switch (c) {
    ...
    default:  // "New" check_nscp also returns performance data
      if (perf.empty()) return check_nt::packet(nscapi::plugin_helper::translateReturn(ret) + _T("&") + message);

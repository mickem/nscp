...
bool test(client::destination_container &source, client::destination_container &) {
  if (source.has_data("log")) gLog = source.get_string_data("log");
  return true;
}
...
void check_nscp::query(const PB::Commands::QueryRequestMessage &request, PB::Commands::QueryResponseMessage &response) { client_.do_query(request, response); }
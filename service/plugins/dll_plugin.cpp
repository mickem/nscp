...
NSCAPI::nagiosReturn nsclient::core::dll_plugin::fetchFacts(const std::string &request, std::string &response) {
  char *buffer = nullptr;
  unsigned int len = 0;
  NSCAPI::nagiosReturn ret = fetchFacts(request.c_str(), static_cast<unsigned int>(request.size()), &buffer, &len);
  if (buffer != nullptr) {
...
NSCAPI::nagiosReturn nsclient::core::dll_plugin::fetchFacts(const char *request_buffer, const unsigned int request_buffer_len, char **returnBuffer,
  ...
  if (!dispatch.entered()) throw plugin_exception(get_alias_or_name(), "Library is unloading");
  try {
    return fFetchFacts(get_id(), request_buffer, request_buffer_len, returnBuffer, returnBuffer_len);
  } catch (...) {

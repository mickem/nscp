...
class plugin_interface : public logging::logging_subscriber {
 ...
 public:
  ...
  // Facts producer: the core hands the module the set ids it wants this round
  // (JSON) and gets the sets the module produced back the same way. See
  ...
  virtual bool route_message(const char *channel, const char *buffer, unsigned int buffer_len, char **new_channel_buffer, char **new_buffer,
                             unsigned int *new_buffer_len) = 0;

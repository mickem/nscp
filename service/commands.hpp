...
class commands : boost::noncopyable {
 ...
 public:
  ...
  std::list<std::string> list_commands() {
    ...
    for (description_list_type::value_type cit : descriptions_) {
      if (commands_.find(cit.first) != commands_.end()) lst.push_back(cit.first);
    }
    return lst;
  }
  std::list<std::string> list_aliases() {
    ...
    for (description_list_type::value_type cit : descriptions_) {
      if (aliases_.find(cit.first) != aliases_.end()) lst.push_back(cit.first);
    }
    return lst;
  }
  ...
  plugin_type get(std::string command) {
    ...
    if (cit != commands_.end()) return (*cit).second;
    cit = aliases_.find(lc);
    if (cit != aliases_.end()) return (*cit).second;
    return plugin_type();
  }
  ...
  static std::string make_key(std::string key) { return boost::algorithm::to_lower_copy(key); }
  void log_error(const char* file, int line, std::string error) { logger_->error("core", file, line, error); }
  void log_error(const char* file, int line, std::string error, std::string command) { logger_->error("core", file, line, error + "for command: " + command); }
  void log_info(const char* file, int line, std::string error, std::string command) { logger_->info("core", file, line, error + "for command: " + command); }

  inline bool have_plugin(unsigned long plugin_id) { return !(plugins_.find(plugin_id) == plugins_.end()); }
};
}  // namespace nsclient
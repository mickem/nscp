...
lua::Lua_State::~Lua_State() { lua_close(L); }
...
bool lua::lua_wrapper::pop_string(std::string &str) {
  ...
  if (!get_string(str, top)) return false;
  pop();
  return true;
}
bool lua::lua_wrapper::pop_raw_string(std::string &str) {
  int top = lua_gettop(L);
  if (top == 0) return false;
  if (!get_raw_string(str, top)) return false;
  pop();
  return true;
}
bool lua::lua_wrapper::pop_function_ref(int &funref) {
  int top = lua_gettop(L);
  if (top == 0) return false;
  if (!is_function(top)) return false;
  funref = luaL_ref(L, LUA_REGISTRYINDEX);
  if (funref == 0) return false;
  return true;
}
...
void lua::lua_wrapper::push_string(std::string s) { lua_pushstring(L, s.c_str()); }
void lua::lua_wrapper::push_boolean(bool b) { lua_pushboolean(L, b ? 1 : 0); }
void lua::lua_wrapper::push_int(int b) { lua_pushinteger(L, b); }
...
int lua::lua_wrapper::size() { return lua_gettop(L); }
bool lua::lua_wrapper::empty() { return size() == 0; }
...
void lua::lua_wrapper::openlibs() { luaL_openlibs(L); }

int lua::lua_wrapper::loadfile(std::string script) { return luaL_loadfile(L, script.c_str()); }

int lua::lua_wrapper::pcall(int nargs, int nresults, int errfunc) { return lua_pcall(L, nargs, nresults, errfunc); }

std::string lua::lua_wrapper::op_string(int pos, std::string def) { return luaL_optstring(L, pos, def.c_str()); }
std::string lua::lua_wrapper::check_string(int pos) { return luaL_checkstring(L, pos); }
...
void lua::lua_wrapper::new_userdata(size_t size) { lua_newuserdata(L, size); }

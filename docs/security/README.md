...
| `fixed_in`      | no       | The release that carries the fix (`0.18.0`), or `next` while it is unreleased (sorts newest; the release process replaces it with the number). Drives the version filter and the newest-first order; omit only for an advisory against a line that has no fix release. |

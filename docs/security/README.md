...
| `fixed_in`      | no       | The release that carries the fix (`0.18.0`), or `next` while it has not shipped yet - shown as *Unreleased*, and renamed with the upgrade notes when the release is cut. Drives the version filter and the newest-first order; omit only for an advisory against a line that has no fix release. |

...
Build your nsclient++ config file
=================================
...
We do recommend that you do not skip this step, as you will have issues later and get frustrated… What is 15mn compared to a great monitoring solution?
If you still can’t afford these 15mn, take the config example in this doc
Syntax
------

?Nsclient++ config file is in the well-known INI Format. Use a correct text editor, like notepad++ or anything that can colorize INI file.
...
Sections:
 * Do not have an explicit end. When a new section start, the previous one end. 
 * can be empty
 * Start in the name with either:
  * /modules
  * /settings/
Comments
--------
...
?In Bold, module needed for Nagios or recommended
 * 0 to disable
 * 1 to enable
...
CheckEventLog			Check for errors and warnings in the event log. This is only supported through NRPE so if you plan to use only NSClient this won’t help you at all
CheckExternalScripts	A simple wrapper to run external scripts and batch files
CheckHelpers			Various helper function to extend other checks. This is also only supported through NRPE
CheckLogFile			Check text files (usually log files) for patterns.
...
DotnetPlugins			Plugin to load and manage plugins written in dot net
GraphiteClient			Act as a Graphite Client over Graphite protocol
LUAScript				Extend NSClient++ with internal Lua scripts.
NRDPClient				Act as a NRDP Client over NRDP protocol
NRPEClient				Act as a NRPE Client over NRPE protocol
NRPEServer				A simple server that listens for incoming NRPE connection and handles them.
NSCAClient				Passive check support over NSCA protocol
NSClientServer			A simple server that listens for incoming NSClient (check_nt) connection and handles them
NSCPClient				A simple client for checking remote NSCP servers.
NSCPServer				A simple server that listens for incoming NSCP connection and handles them
PythonScript			Execute internal script written in Python (requires python 2.7 to be installed)
Scheduler				A scheduler which schedules checks at regular intervals
SimpleCache				Caches results for later checking
SimpleFileWriter		FileWriters results for later checking
======================= ==================
...
port = 5666
NSCA Client
Nsclient++ will send trap information to Nagios. The main difference is that Nagios is just waiting for these informations and so doesn’t poll for them. This can avoid a lot of load by not polling for nothing. Load is also distributed on all hosts and not Nagios centric.
...
verify mode = none
External scripts
This allow to call arbitrary program/script through any protocol and get result back. This enhanced a LOT monitoring solution, and so you want that.
?Alias allows to shorten commands sent by preparing command line with both dynamics and static arguments.
...
Eventlogs
They are a great source of informations for monitoring. Allow real-time monitoring and extended filtering. As the subject itself is a bit complicated, start basic by grabbing all and exclude after.
...
Yes, it’s really free, only limited to 8 computers at same time. Yes, it’s graphical and works great, even on Windows Core 2012. It can deploy both domain joined and workgroup, only need to provide credentials and source computer must have windows share access (firewall).
...
net stop nscp
net start nscp
?This example covers windows in 64 bit. If you have x86 Windows, you need either 2 packages or add intelligence to install the good one. Successfully tested on:
 * Windows Server 2003
...
?You can use Process monitor to get access denied logs.

...
The size of the historical buffer is controlled by the `default buffer length` setting under
`/settings/system/windows` (alias `[/settings/system/windows]`). The default is `1h`, meaning the
last hour of samples is retained. The buffer size puts an upper bound on the time windows you can
use:
...
as Task Manager at the moment the check is executed, and very short spikes that fall between
collection ticks may be missed entirely.

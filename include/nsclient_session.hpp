...
namespace nsclient_session {
	...
	public:
		...
		virtual int session_inject(std::wstring command, std::wstring arguments, std::wstring splitter, bool escape, std::wstring &msg, std::wstring & perf) = 0;
	};
		...
		void handle_message(remote_channel::local_message_type &msg) {
			if (msg.command == message_inject) {
				...
				if (msg.arguments.size() == 4) {
					try {
						ret = handler_->session_inject(msg.arguments[0], msg.arguments[1], msg.arguments[2], msg.arguments[2]==_T("1"), message, perf);
					} catch (...) {

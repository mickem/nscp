...
class remote_channel {
 ...
 public:
  ...
  HANDLE getSignalHandle() { return signal_event; }

  local_message_type read() {
    if (!shared_memory.isOpen()) throw session_exception(_T("Invalid session: no memory (cant read message)!"));
    if (!write_event.isOpen()) throw session_exception(_T("Invalid session: no signal (cant read message)!"));

    local_message_type msg;
    shared_message_type *tmpMsg = reinterpret_cast<shared_message_type *>(shared_memory.getBufferUnsafe());
    if (tmpMsg->version != message_version) throw session_exception(_T("Invalid message version"));
  ...
  void write(local_message_type msg, DWORD timeout = 5000L) {
    if (!shared_memory.isOpen()) throw session_exception(_T("Invalid session: no memory (cant send message)!"));
    if (!signal_event.isOpen()) throw session_exception(_T("Invalid session: no signal (cant send message)!"));
  ...
  bool exists() const { return event_handler::exists(signal_event.get_name()); }
  static bool exists(std::wstring name) { return event_handler::exists(channel_prefix + name + _T("_signal")); }
  std::wstring get_name() const {
    return _T("channel-") + channel_name_ + _T("= {") + write_event.get_name() + _T(", ") + signal_event.get_name() + _T(", ") + shared_memory.get_name() +
           _T("}");
  }

 private:
  inline wchar_t *pushString(wchar_t *strBuffer) { return strBuffer + wcslen(strBuffer) + 1; }
  void error(std::wstring file, unsigned int line, std::wstring msg) {
    if (instance_) instance_->error(file, line, msg);
  }
};
...
class responder {
 ...
 public:
  ...
  DWORD threadProc(LPVOID lpParameter) {
    ...
    if (handles[1] == NULL)
      ...
      while (true) {
        ...
        if (waitStatus == WAIT_OBJECT_0) break;  // exit event
  ...
  void exitThread(void) {
    if (instance_ == NULL) throw session_exception(_T("No instance exists cant close the session, make sure the thread has been started!"));
    if (hStopEvent_ == NULL) {
...
class shared_session : public session_interface {
 ...
 public:
  shared_session(std::wstring session_id, session_handler_interface *handler)
      : responder_(_T("shmem-responder-thread")), client_id_(session_id), handler_(handler), message_id_(0), master_id_(_T("server")) {}
  ~shared_session() {
    try {
      close_session();
    } catch (session_exception &e) {
  ...
  void set_handler(session_handler_interface *handler) { handler_ = handler; }
  std::wstring get_client_id() const { return client_id_; }
  virtual std::wstring get_name() { return client_id_; }

  void error(std::wstring file, unsigned int line, std::wstring msg) {
    if (handler_) handler_->session_error(file, line, msg);
  }
  void info(std::wstring file, unsigned int line, std::wstring msg) {
    if (handler_) handler_->session_info(file, line, msg);
  }
  ...
  void remove_client(std::wstring client_id) {
    ...
    if (it == remote_channels_.end()) throw session_exception(_T("Failed to detach client: ") + client_id + _T(": client was not found."));
  ...
  bool handle_raw_message() {
    ...
    if (msg.command == message_attach) {
      if (client_channel_.get() != NULL) throw session_exception(_T("Cant attach ") + msg.sender + _T(" to a client session!"));
      on_attach_client(msg.sender);
    } else if (msg.command == message_detach) {
      if (client_channel_.get() != NULL) throw session_exception(_T("Cant detach ") + msg.sender + _T(" to a client session!"));
      on_detach_client(msg.sender);
    } else if (msg.command == message_master_shutdown) {
      if (client_channel_.get() == NULL) throw session_exception(_T("Cant handle master shutdown from ") + msg.sender + _T(" not a client session!"));
      on_master_shutdown(msg.sender);
      // return false;
    } else if (msg.command == message_master_attach) {
      error(__FILEW__, __LINE__, _T("Got attach from master!"));
      if (client_channel_.get() == NULL) throw session_exception(_T("Cant handle master attach from ") + msg.sender + _T(" not a client session!"));
  ...
  unsigned int send_server(remote_channel::local_message_type &msg) {
    ...
    if (master_channel_.get() == NULL) throw session_exception(_T("Master session cant be down!"));
  ...
  void send_clients(remote_channel::local_message_type &msg) {
    if (client_channel_.get() != NULL) throw session_exception(_T("Server channels cant have client sessions..."));
    if (remote_channels_.empty()) return;
    msg.message_id = message_id_++;
    msg.sender = client_id_;
    for (handle_type::iterator it = remote_channels_.begin(); it != remote_channels_.end(); ++it) {
      try {
  ...
  void send_client(std::wstring client, remote_channel::local_message_type &msg) {
    if (client_channel_.get() != NULL) throw session_exception(_T("Server channels cant have client session..."));
    if (remote_channels_.empty()) return;
    msg.sender = client_id_;
...
class shared_client_session : public shared_session {
 ...
 public:
  shared_client_session(std::wstring channel, session_handler_interface *handler) : shared_session(channel, handler), channel_id_(channel) {}
  virtual ~shared_client_session() {}

  void create_new_session() { throw session_exception(_T("Client channel cant create master session!")); }
  ...
  void wait_for_my_channel() {
    if (!remote_channel::exists(client_id_)) {
      ...
      for (; i < 10; i++) {
        ...
        if (remote_channel::exists(client_id_)) break;
      }
      if (i == 10) throw session_exception(_T("Failed to open shared mempory (was never created) for: ") + client_id_);
    }
  }
  ...
  virtual void attach_to_session(std::wstring channel) {
    ...
    open_master_channel();
    wait_for_my_channel();
    if (master_channel_.get() == NULL) open_master_channel();
    start_my_channel();
    info(__FILEW__, __LINE__, _T("Created new client: ") + client_channel_->get_name());
  }
  bool session_exists() const { return remote_channel::exists(master_id_); }
  ...
  void handle_message(remote_channel::local_message_type &msg) {
    ...
    if (it != wait_list_.end()) {
      ...
      if (handler_) handler_->session_log_message(strEx::stoi(msg.arguments[0]), msg.arguments[1].c_str(), strEx::stoi(msg.arguments[2]), msg.arguments[3]);
    } else {
  ...
  std::pair<std::wstring, std::wstring> get_client_name() {
    ...
    if (response.arguments.size() != 2) return std::pair<std::wstring, std::wstring>(_T("Unknown: Failed to get service name"), _T(""));
  ...
  int inject(std::wstring command, std::wstring arguments, TCHAR splitter, bool escape, std::wstring &message, std::wstring &perf) {
    ...
    unsigned int msg_id = send_server(msg);
    remote_channel::local_message_type response = wait_reply(msg_id);
    if (response.arguments.size() != 3) return -1;
    int code = strEx::stoi(response.arguments[0]);
    message = response.arguments[1];
    perf = response.arguments[2];
    return code;
  }
  bool hasMaster() const { return master_channel_.get() != NULL && client_channel_.get() != NULL; }
  void send_attach() { send_server(message_attach); }
  void send_detach() { send_server(message_detach); }
  virtual void close_session(DWORD dwTimeout = 5000) {
    try {
      send_detach();
    } catch (session_exception &e) {
...
class shared_server_session : public shared_session {
 ...
 public:
  ...
  void attach_to_session() { throw session_exception(_T("Master channel cant attach to a master session!")); }
  bool session_exists() const { return remote_channel::exists(client_id_); }
  ...
  bool re_attach_client(DWORD dwSessionId) {
    ...
    if (it != remote_channels_.end()) return true;
  ...
  bool hasClients() const { return master_channel_.get() != NULL && !remote_channels_.empty(); }
  void send_master_shutdown() { send_clients(message_master_shutdown); }
  void send_master_attach(std::wstring client) { send_client(client, message_master_attach); }
  virtual void close_session(DWORD dwTimeout = 5000) {
    send_master_shutdown();
    shared_session::close_session(dwTimeout);
  }
  virtual void on_master_shutdown(std::wstring client_id) { error(__FILEW__, __LINE__, client_id + _T(" has shut down?!?!")); }
  virtual void on_master_attach(std::wstring client_id) { error(__FILEW__, __LINE__, client_id + _T(" has attached it self?!?!")); }
};
}  // namespace nsclient_session

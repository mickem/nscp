...
class shared_memory_handler {
 ...
 public:
   ...
   */
  shared_memory_handler(std::wstring name = _T(""), DWORD dwSize = 4096) : hMem(NULL), name_(name), pBuffer(NULL), dwSize_(dwSize) { create(); }
   ...
   */
  virtual ~shared_memory_handler() { close(); }
  void close() {
   ...
   */
  operator LPCTSTR() const { return pBuffer; }
  LPTSTR getBufferUnsafe() const { return pBuffer; }
  bool isOpen() { return hMem != NULL && pBuffer != NULL; }

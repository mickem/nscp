...
struct simple_bool_binary_operator_impl : public binary_operator_impl {
  node_type evaluate(evaluation_context errors, const node_type left, const node_type right) const {
    ...
    if (helpers::type_is_int(ltype) && helpers::type_is_int(rtype)) return factory::create_num(eval_int(ltype, errors, left, right));

    if (helpers::type_is_float(ltype) && helpers::type_is_float(rtype)) return factory::create_num(eval_float(ltype, errors, left, right));
    ...
    if (helpers::type_is_int(type)) return factory::create_num(eval_int(type, errors, left, right));
    if (helpers::type_is_float(type)) return factory::create_num(eval_float(type, errors, left, right));
    if (type == type_string) return factory::create_num(eval_string(type, errors, left, right));
...
struct eval_helper {
  ...
  eval_helper(evaluation_context errors, const node_type left, const node_type right)
      : left(left), right(right), errors(errors), ltype(left->get_type()), rtype(right->get_type()) {}
...
struct operator_not_in : public simple_bool_binary_operator_impl {
  value_container eval_int(value_type type, evaluation_context errors, const node_type left, const node_type right) const {
    ...
    for (node_type itm : right->get_list_value(errors)) {
      if (itm->get_int_value(errors) == val) return value_container::create_int(false, lhs.is_unsure);
    }
  ...
  value_container eval_float(value_type, evaluation_context errors, const node_type left, const node_type right) const {
    ...
    for (node_type itm : right->get_list_value(errors)) {
      if (itm->get_float_value(errors) == val) return value_container::create_int(false, lhs.is_unsure);
    }
    return value_container::create_int(true, lhs.is_unsure);
  }
  value_container eval_string(value_type, evaluation_context errors, const node_type left, const node_type right) const {
    ...
    for (node_type itm : right->get_list_value(errors)) {
      if (itm->get_string_value(errors) == val) return value_container::create_int(false, lhs.is_unsure);
    }
    return value_container::create_int(true, lhs.is_unsure);
  };
};
struct operator_in : public simple_bool_binary_operator_impl {
  ...
  value_container eval_float(value_type, evaluation_context errors, const node_type left, const node_type right) const {
    ...
    double val = lhs.get_float();
    for (node_type itm : right->get_list_value(errors)) {
      if (itm->get_float_value(errors) == val) return value_container::create_int(true, lhs.is_unsure);
    }
    return value_container::create_int(false, lhs.is_unsure);
  }
  value_container eval_string(value_type, evaluation_context errors, const node_type left, const node_type right) const {
    ...
    std::string val = lhs.get_string();
    for (node_type itm : right->get_list_value(errors)) {
      if (itm->get_string_value(errors) == val) return value_container::create_int(true, lhs.is_unsure);
    }
    return value_container::create_int(false, lhs.is_unsure);
  };
};
...
struct simple_bool_unary_operator_impl : public unary_operator_impl {
  node_type evaluate(evaluation_context errors, const node_type subject) const {
    ...
    if (helpers::type_is_int(type)) return eval_int(type, errors, subject) ? factory::create_true() : factory::create_false();
    if (type == type_string) return eval_string(type, errors, subject) ? factory::create_true() : factory::create_false();
...
struct operator_not : public unary_operator_impl, binary_function_impl {
  ...
  operator_not() {}
  node_type evaluate(evaluation_context errors, const node_type subject) const { return evaluate(subject->get_type(), errors, subject); }
  node_type evaluate(value_type type, evaluation_context errors, const node_type subject) const {
    if (type == type_bool) return subject->get_int_value(errors) ? factory::create_false() : factory::create_true();
    if (type == type_int) return factory::create_int(-subject->get_int_value(errors));
    if (type == type_date) {
      long long now = constants::get_now();
...
op_factory::bin_fun_type op_factory::get_binary_function(evaluation_context errors, std::string name, const node_type subject) {
  if (name == "convert" || name == "auto_convert") return op_factory::bin_fun_type(new operator_impl::function_convert(errors, subject));
  if (name == "neg") return op_factory::bin_fun_type(new operator_impl::operator_not(subject));
...
op_factory::un_op_type op_factory::get_unary_operator(operators op) {
  // op_inv, op_not
  if (op == op_not) return op_factory::un_op_type(new operator_impl::operator_not());
  std::cout << "======== UNHANDLED OPERATOR\n";

...
node_type unary_op::evaluate(evaluation_context errors) const {
  ...
  value_type wanted_type = helpers::get_return_type(op, type_invalid);
...
bool unary_op::bind(object_converter errors) { return subject->bind(errors); }
value_type unary_op::infer_type(object_converter converter, value_type suggestion) {
  value_type wanted_type = subject->infer_type(converter, suggestion);
  return helpers::get_return_type(op, wanted_type);
}
value_type unary_op::infer_type(object_converter converter) {
  value_type wanted_type = subject->infer_type(converter);
  return helpers::get_return_type(op, wanted_type);
}
...
bool unary_op::static_evaluate(evaluation_context errors) const { return subject->static_evaluate(errors); }
bool unary_op::require_object(evaluation_context errors) const { return subject->require_object(errors); }
}  // namespace where
}  // namespace parsers
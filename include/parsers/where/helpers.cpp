...
bool type_is_string(value_type type) { return type == type_string || (type >= type_custom_string && type < type_custom_string_end); }
...
node_type add_convert_node(node_type subject, value_type newtype) {
  ...
  if (type == newtype) return subject;
...
value_type infer_binary_type(object_converter factory, node_type &left, node_type &right) {
  ...
  if (lt == rt) return lt;
  if (type_is_float(lt) && type_is_int(rt)) rt = right->infer_type(factory, lt);
  if (type_is_float(rt) && type_is_int(lt)) lt = left->infer_type(factory, rt);
  if (lt == rt) return lt;
...
bool is_lower(operators op) { return op == op_le || op == op_lt; }
bool is_upper(operators op) { return op == op_ge || op == op_gt; }

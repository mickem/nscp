...
namespace parsers {
	namespace where {
		...
		expression_ast unary_fun::evaluate(filter_handler handler, value_type type) const {
			...
			handler->error("Missing function binding: " + name + "bound: " + strEx::s::xtos(is_bound()));
		...
		bool unary_fun::bind(value_type type, filter_handler handler) {
			try {
				if (handler->has_function(type, name, &subject)) {
					...
					if (!function_id) {
						handler->error("Failed to bind function: " + name);
						return false;
					}
					return true;
				}
				...
				if (!i_fn) {
					handler->error("Failed to create function: " + name);
					return false;
				}
				return true;
			} catch (...) {

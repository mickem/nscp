...
std::string unary_fun::to_string(evaluation_context errors) const {
  if (function) return name + "(" + function->evaluate(type_string, errors, subject)->to_string(errors) + ")";
...
bool unary_fun::require_object(evaluation_context context) const { return subject->require_object(context); }

...
struct read_protocol : public boost::noncopyable {
  ...
  bool has_data() { return current_state_ == got_request; }
  ...
  void log_debug(std::string file, int line, std::string msg) const { handler_->log_debug("check_nt", file, line, msg); }
  void log_error(std::string file, int line, std::string msg) const { handler_->log_error("check_nt", file, line, msg); }
};

...
namespace check_nt {
	namespace server {
		...
		connection::connection(boost::asio::io_service& io_service, boost::shared_ptr<check_nt::server::handler> handler)
			: strand_(io_service)
			, handler_(handler)
			, parser_(handler)
			, timer_(io_service)
		...
		connection* factories::create(boost::asio::io_service& io_service, boost::asio::ssl::context &context, boost::shared_ptr<check_nt::server::handler> handler, bool use_ssl) {
			if (use_ssl)
				return create_ssl(io_service, context, handler);
			return create_tcp(io_service, handler);
		}
		connection* factories::create_tcp(boost::asio::io_service& io_service, boost::shared_ptr<check_nt::server::handler> handler) {
			return new tcp_connection(io_service, handler);
		}
		connection* factories::create_ssl(boost::asio::io_service& io_service, boost::asio::ssl::context &context, boost::shared_ptr<check_nt::server::handler> handler) {
			...
			if (!e) {
				...
				while (begin != end) {
					...
					if (result) {
						...
						} catch (check_nt::check_nt_packet_exception &e) {

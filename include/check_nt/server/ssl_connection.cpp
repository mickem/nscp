...
namespace check_nt {
	namespace server {
		...
		ssl_connection::ssl_connection(boost::asio::io_service& io_service, boost::asio::ssl::context &context, boost::shared_ptr<check_nt::server::handler> handler)
		...
		void ssl_connection::start() {
			handler_->log_debug(__FILE__, __LINE__, _T("starting ssl_connection"));
			socket_.async_handshake(boost::asio::ssl::stream_base::server,
				strand_.wrap(
					boost::bind(&connection::handle_handshake, shared_from_this(), boost::asio::placeholders::error)
					)
				);
			handler_->log_debug(__FILE__, __LINE__, _T("starting ssl_connection (started)"));
		}

		void ssl_connection::handle_handshake(const boost::system::error_code& error) {
			handler_->log_debug(__FILE__, __LINE__, _T("handle_handshake ssl_connection"));
			if (!error)
				connection::start();
			else {

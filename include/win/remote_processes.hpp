...
BOOL _WTSEnumerateSessions(HANDLE hServer, DWORD Reserved, DWORD Version, PWTS_SESSION_INFO *ppSessionInfo, DWORD *pCount) {
  if (lpfnWTSEnumerateSessions == NULL) {
    ...
    if (hMod != NULL) lpfnWTSEnumerateSessions = (LPFN_WTSEnumerateSessions)GetProcAddress(hMod, "WTSEnumerateSessionsW");
  }
  if (lpfnWTSEnumerateSessions == NULL) return FALSE;
...
DWORD _WTSGetActiveConsoleSessionId() {
  if (lpfnWTSGetActiveConsoleSessionId == NULL) {
    HMODULE hMod = GetModuleHandle(_T("kernel32.dll"));
    if (hMod != NULL) lpfnWTSGetActiveConsoleSessionId = (LPFN_WTSGetActiveConsoleSessionId)GetProcAddress(hMod, "WTSGetActiveConsoleSessionId");
  }
  if (lpfnWTSGetActiveConsoleSessionId == NULL) return 0;
  return lpfnWTSGetActiveConsoleSessionId();
}
DWORD getActiveSessionId() { return _WTSGetActiveConsoleSessionId(); }
...
BOOL _WTSQueryUserToken(ULONG SessionId, PHANDLE phToken) {
  ...
  if (lpfWTSQueryUserToken == NULL) return FALSE;

...
class NativeExternalPDH : public PDH::impl_interface {
 ...
 public:
  virtual pdh_error PdhLookupPerfIndexByName(LPCTSTR szMachineName, LPCTSTR szName, DWORD *dwIndex) {
    if (pPdhLookupPerfIndexByName == NULL) throw pdh_exception("Failed to initialize PdhLookupPerfIndexByName");
  ...
  virtual pdh_error PdhLookupPerfNameByIndex(LPCTSTR szMachineName, DWORD dwNameIndex, LPTSTR szNameBuffer, LPDWORD pcchNameBufferSize) {
    if (pPdhLookupPerfNameByIndex == NULL) throw pdh_exception("Failed to initialize PdhLookupPerfNameByIndex");
  ...
  virtual pdh_error PdhExpandCounterPath(LPCTSTR szWildCardPath, LPTSTR mszExpandedPathList, LPDWORD pcchPathListLength) {
    if (pPdhExpandCounterPath == NULL) throw pdh_exception("Failed to initialize PdhLookupPerfNameByIndex");
  ...
  virtual pdh_error PdhGetCounterInfo(PDH::PDH_HCOUNTER hCounter, BOOLEAN bRetrieveExplainText, LPDWORD pdwBufferSize, PDH_COUNTER_INFO *lpBuffer) {
    if (pPdhGetCounterInfo == NULL) throw pdh_exception("Failed to initialize PdhGetCounterInfo");
  ...
  virtual pdh_error PdhAddCounter(PDH::PDH_HQUERY hQuery, LPCWSTR szFullCounterPath, DWORD_PTR dwUserData, PDH::PDH_HCOUNTER *phCounter) {
    if (pPdhAddCounter == NULL) throw pdh_exception("Failed to initialize PdhAddCounter");
  ...
  virtual pdh_error PdhAddEnglishCounter(PDH::PDH_HQUERY hQuery, LPCWSTR szFullCounterPath, DWORD_PTR dwUserData, PDH::PDH_HCOUNTER *phCounter) {
    if (pPdhAddEnglishCounter == NULL) throw pdh_exception("PdhAddEnglishCounter is only available on Vista and later you need to use localized counters.");
  ...
  virtual pdh_error PdhRemoveCounter(PDH::PDH_HCOUNTER hCounter) {
    if (pPdhRemoveCounter == NULL) throw pdh_exception("Failed to initialize PdhRemoveCounter");
  ...
  virtual pdh_error PdhGetRawCounterValue(PDH::PDH_HCOUNTER hCounter, LPDWORD dwFormat, PPDH_RAW_COUNTER pValue) {
    if (pPdhGetRawCounterValue == NULL) throw pdh_exception("Failed to initialize PdhGetRawCounterValue");
  ...
  virtual pdh_error PdhGetFormattedCounterValue(PDH_HCOUNTER hCounter, DWORD dwFormat, LPDWORD lpdwType, PPDH_FMT_COUNTERVALUE pValue) {
    if (pPdhGetFormattedCounterValue == NULL) throw pdh_exception("Failed to initialize PdhGetFormattedCounterValue");
  ...
  virtual pdh_error PdhOpenQuery(LPCWSTR szDataSource, DWORD_PTR dwUserData, PDH::PDH_HQUERY *phQuery) {
    if (pPdhOpenQuery == NULL) throw pdh_exception("Failed to initialize PdhOpenQuery");
    return PDH::pdh_error(pPdhOpenQuery(szDataSource, dwUserData, phQuery));
  }
  virtual pdh_error PdhCloseQuery(PDH_HQUERY hQuery) {
    if (pPdhCloseQuery == NULL) throw pdh_exception("Failed to initialize PdhCloseQuery");
    return PDH::pdh_error(pPdhCloseQuery(hQuery));
  }
  virtual pdh_error PdhCollectQueryData(PDH_HQUERY hQuery) {
    if (pPdhCollectQueryData == NULL) throw pdh_exception("Failed to initialize PdhCollectQueryData");
  ...
  virtual pdh_error PdhValidatePath(LPCWSTR szFullPathBuffer, bool force_reload) {
    if (pPdhValidatePath == NULL) throw pdh_exception("Failed to initialize PdhValidatePath");
  ...
  virtual pdh_error PdhEnumObjects(LPCWSTR szDataSource, LPCWSTR szMachineName, LPWSTR mszObjectList, LPDWORD pcchBufferSize, DWORD dwDetailLevel,
                                   BOOL bRefresh) {
    if (pPdhEnumObjects == NULL) throw pdh_exception("Failed to initialize PdhEnumObjects");
  ...
  virtual pdh_error PdhEnumObjectItems(LPCWSTR szDataSource, LPCWSTR szMachineName, LPCWSTR szObjectName, LPWSTR mszCounterList, LPDWORD pcchCounterListLength,
                                       LPWSTR mszInstanceList, LPDWORD pcchInstanceListLength, DWORD dwDetailLevel, DWORD dwFlags) {
    if (pPdhEnumObjectItems == NULL) throw pdh_exception("Failed to initialize PdhEnumObjectItems");
    return PDH::pdh_error(pPdhEnumObjectItems(szDataSource, szMachineName, szObjectName, mszCounterList, pcchCounterListLength, mszInstanceList,
  ...
  virtual pdh_error PdhExpandWildCardPath(LPCTSTR szDataSource, LPCTSTR szWildCardPath, LPWSTR mszExpandedPathList, LPDWORD pcchPathListLength, DWORD dwFlags) {
    if (pPdhExpandWildCardPath == NULL) throw pdh_exception("Failed to initialize PdhExpandWildCardPath");

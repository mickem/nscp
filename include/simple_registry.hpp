...
class registry_exception {
 ...
 public:
  ...
  registry_exception(std::wstring path, std::wstring key, std::string what)
      : what_(utf8::cvt<std::string>(path) + "." + utf8::cvt<std::string>(key) + ": " + what) {}
...
class registry_key {
 ...
 public:
  ...
  std::wstring get_string(std::wstring key, DWORD buffer_length = 2048) {
    ...
    if (cbData >= buffer_length || cbData < 0) throw registry_exception(path_, key, "Failed to get value: buffer to small");
    bData_[cbData] = 0;
    if (type == REG_SZ) {
  ...
  DWORD get_int(std::wstring key) {
    ...
    if (lRet != ERROR_SUCCESS) throw registry_exception(path_, key, "Failed to get value: " + error::format::from_system(lRet));
    if (type != REG_DWORD) throw registry_exception(path_, key, "Unknown type (not a DWORD)");
    return ret;
  }
  ...
  std::list<std::wstring> get_keys(DWORD buffer_length = 2048) {
    ...
    if (lRet != ERROR_SUCCESS) throw registry_exception(path_, "Failed to query key info: " + error::format::from_system(lRet));
    if (cSubKeys == 0) return ret;
    delete[] buffer_;
...
static HKEY parseHKEY(std::wstring key) {
  if (key == _T("HKEY_CLASSES_ROOT")) return HKEY_CLASSES_ROOT;
  if (key == _T("HKEY_LOCAL_MACHINE")) return HKEY_LOCAL_MACHINE;
  if (key == _T("HKEY_CURRENT_USER")) return HKEY_CURRENT_USER;

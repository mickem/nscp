...
class checks {
 public:
  ...
  static bool is_file(std::string path) { return fs::is_regular_file(path); }
...
class meta {
 public:
  ...
  static std::string get_filename(std::string file) { return get_filename(fs::path(file)); }
...
class patterns {
 public:
  ...
  static fs::path combine_pattern(pattern_type pattern) { return pattern.first / pattern.second; }
...
#else
    if (boost::filesystem::is_regular_file(fullpath)) return fullpath;

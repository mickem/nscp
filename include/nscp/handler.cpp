...
std::list<nscp::packet> nscp::handler::process_all(const std::list<nscp::packet> &packets) {
	...
	BOOST_FOREACH(const nscp::packet &packet, packets) {
		std::list<nscp::packet> tmp = process(packet);
...
std::list<nscp::packet> nscp::handler::process(const nscp::packet &packet) {
	std::list<nscp::packet> result;
	...
	if (nscp::checks::is_query_request(packet)) {
		...
		result.push_back(nscp::factory::create_error(_T("Unknown packet: ") + packet.to_wstring()));
	}
	return result;
}
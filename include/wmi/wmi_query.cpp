...
struct identidy_container {
  identidy_container(std::wstring domain, std::wstring username, std::wstring password) : domain(domain), username(username), password(password) { setup(); }
  identidy_container(const identidy_container &other) : domain(other.domain), username(other.username), password(other.password) { setup(); }
...
CComPtr<IWbemServices> &wmi_service::get() {
  if (!is_initialized) {
    ...
    if (FAILED(hr)) throw wmi_exception(hr, "CoCreateInstance for CLSID_WbemAdministrativeLocator failed!");
    ...
    if (FAILED(hr)) throw wmi_exception(hr, "ConnectServer failed: namespace=" + ns + ", user=" + username);
...
row_enumerator classes::get() {
  ...
  HRESULT hr = instance.get()->CreateClassEnum(
      strSuperClass, WBEM_FLAG_DEEP | WBEM_FLAG_USE_AMENDED_QUALIFIERS | WBEM_FLAG_RETURN_IMMEDIATELY | WBEM_FLAG_FORWARD_ONLY, NULL, &ret.enumerator_obj);
...
row_enumerator instances::get() {
  ...
  CComBSTR strSuperClass = utf8::cvt<std::wstring>(super_class).c_str();
  CComPtr<IEnumWbemClassObject> enumerator;
  HRESULT hr = instance.get()->CreateInstanceEnum(
      strSuperClass, WBEM_FLAG_SHALLOW | WBEM_FLAG_USE_AMENDED_QUALIFIERS | WBEM_FLAG_RETURN_IMMEDIATELY | WBEM_FLAG_FORWARD_ONLY, NULL, &ret.enumerator_obj);
  if (FAILED(hr)) throw wmi_exception(hr, "CreateInstanceEnum failed: " + ComError::getComError(hr));
  return ret;
}

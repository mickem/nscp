...
struct base_url {
  ...
  string_type to_string() const {
    ...
    if (port != 0) ss << traits::port_prefix() << port;
    ss << path;
    return ss.str();
  }
  ...
  inline std::string get_port_string() const { return boost::lexical_cast<std::string>(port); }
...
inline url parse(const std::string &url_s, unsigned int default_port = 0) {
  ...
  if (ret.protocol != "ini" && ret.protocol != "registry") {
    if ((path_i != url_s.end()) && (*path_i == ':')) {
      std::string::const_iterator port_b = path_i;
      ++port_b;

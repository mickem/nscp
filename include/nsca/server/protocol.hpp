...
struct read_protocol : public boost::noncopyable {
  ...
  bool wants_data() { return current_state_ == sent_iv; }
  ...
  void log_debug(std::string file, int line, std::string msg) const { handler_->log_debug("nsca", file, line, msg); }
  void log_error(std::string file, int line, std::string msg) const { handler_->log_error("nsca", file, line, msg); }
};

...
TEST(format, format_date_from_time_t_custom_format) {
  EXPECT_EQ(str::format::format_date(static_cast<std::time_t>(0), "%Y"), "1970");
}
...
TEST(format, decode_byte_units_string_throws_on_empty) {
  EXPECT_THROW(str::format::decode_byte_units(""), boost::bad_lexical_cast);
}

TEST(format, decode_byte_units_string_throws_on_no_digits) {
  EXPECT_THROW(str::format::decode_byte_units("KB"), boost::bad_lexical_cast);
}
...
TEST(format, format_byte_units_with_unit_terabyte) {
  EXPECT_EQ(str::format::format_byte_units(1024LL * 1024LL * 1024LL * 1024LL, "T"), "1");
}

// format_byte_units<T>(value, unit) with P (petabyte) unit
TEST(format, format_byte_units_with_unit_petabyte) {
  EXPECT_EQ(str::format::format_byte_units(1024LL * 1024LL * 1024LL * 1024LL * 1024LL, "P"), "1");
}

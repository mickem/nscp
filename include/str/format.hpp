...
inline std::string format_date(std::time_t time, std::string format = "%Y-%m-%d %H:%M:%S") { return format_date(boost::posix_time::from_time_t(time), format); }
...
inline unsigned long long filetime_to_time(unsigned long long filetime) { return (filetime - (SECS_BETWEEN_EPOCHS * SECS_TO_100NS)) / SECS_TO_100NS; }

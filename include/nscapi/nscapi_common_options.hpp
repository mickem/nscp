...
// The shared password the *inbound* protocols verify a caller against - the WEB
...
#define DEFAULT_PASSWORD_DESC                                                                                                        \
  ...
  "to verify against, not key material: NSCA encrypts with its shared secret instead of verifying it, so it keeps its own key "        \

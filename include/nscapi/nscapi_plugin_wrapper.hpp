...
struct basic_wrapper_static {
  ...
  static void set_alias(const char *default_alias, const char *alias) { nscapi::plugin_singleton->get_core()->set_alias(default_alias, alias); }
  static int NSLoadModule() { return NSCAPI::api_return_codes::hasFailed; }
...
struct facts_wrapper {
  ...
  int NSFetchFacts(const char *request_buffer, const unsigned int request_buffer_len, char **response_buffer, unsigned int *response_buffer_len) {
    try {
      ...
      std::string reply;
      const NSCAPI::nagiosReturn retCode = instance->fetchFacts(request, reply);
      helpers::wrap_string(reply, response_buffer, response_buffer_len);
      return retCode;
    } catch (const std::exception &e) {

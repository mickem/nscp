...
struct NSCAPI_EXPORT filter_object {
 ...
 public:
  ...
  filter_object(std::string syntax_top, std::string syntax_detail, std::string target)
      : debug(false), escape_html(false), syntax_top(syntax_top), syntax_detail(syntax_detail), target(target), severity(-1) {}
  ...
  const char *filter_string() { return filter_string_.c_str(); }
  ...
  void set_severity(std::string severity_) { severity = nscapi::plugin_helper::translateReturn(severity_); }

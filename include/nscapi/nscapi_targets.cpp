...
void nscapi::targets::target_object::read(nscapi::settings_helper::settings_impl_interface_ptr proxy, bool, bool is_sample) {
  ...
  root_path.add_key()
      ...
      .add_bool("allow host override", sh::bool_fun_key([this](auto key) { this->set_property_bool("allow host override", key); }, false),
                ...
                "request names, or under transport settings the request names. Off by default: when a request moves the destination (`host=`, `port=`, "
                "`address=`), reroutes it through a proxy of its own (`proxy=`, `no-proxy=`) or changes how the connection is protected (`verify=`, "
                "`insecure=`, `ca=`, `security=`, `tls-version=`, the certificate and cipher keys) and the password or token going with it is the one "
                "configured here, the call is refused, since otherwise anyone able to run the module's commands could direct the configured credentials "
                "to a host of their choosing, or strip the check that the host answering is the configured one. This setting is only consulted in that "
                "case: a target with no credentials, a request that supplies its own password/token, and a request that changes none of those are all "
                "unaffected. To reach another server with its own credentials, prefer configuring it as a separate target and selecting it with "
                "`target=`.",
                true);

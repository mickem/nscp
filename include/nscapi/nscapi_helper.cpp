...
bool nscapi::report::matches(unsigned int report, NSCAPI::nagiosReturn code) {
  return ((code == NSCAPI::query_return_codes::returnOK && ((report & REPORT_OK) == REPORT_OK)) ||
          ...
          ((code != NSCAPI::query_return_codes::returnOK) && (code != NSCAPI::query_return_codes::returnCRIT) &&
           (code != NSCAPI::query_return_codes::returnWARN) && (code != NSCAPI::query_return_codes::returnUNKNOWN)));
}
...
bool nscapi::logging::matches(NSCAPI::log_level::level level, NSCAPI::nagiosReturn code) { return code <= level; }
...
bool nscapi::plugin_helper::isNagiosReturnCode(NSCAPI::nagiosReturn code) {
  return ((code == NSCAPI::query_return_codes::returnOK) || (code == NSCAPI::query_return_codes::returnWARN) ||
          (code == NSCAPI::query_return_codes::returnCRIT) || (code == NSCAPI::query_return_codes::returnUNKNOWN));
}
bool nscapi::plugin_helper::isMyNagiosReturn(NSCAPI::nagiosReturn code) {
  return code == NSCAPI::query_return_codes::returnCRIT || code == NSCAPI::query_return_codes::returnOK || code == NSCAPI::query_return_codes::returnWARN ||
         code == NSCAPI::query_return_codes::returnUNKNOWN;
}
NSCAPI::nagiosReturn nscapi::plugin_helper::int2nagios(int code) { return code; }
int nscapi::plugin_helper::nagios2int(NSCAPI::nagiosReturn code) { return code; }
void nscapi::plugin_helper::escalteReturnCodeToCRIT(NSCAPI::nagiosReturn &currentReturnCode) { currentReturnCode = NSCAPI::query_return_codes::returnCRIT; }

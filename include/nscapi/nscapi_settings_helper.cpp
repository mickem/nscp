...
class typed_key : public key_interface {
 ...
 public:
  ...
  typed_key(store_functor *store_functor, const std::string &default_value)
      : has_default_(true), default_value_(default_value), store_functor_(store_ptr(store_functor)) {}
...
struct path_info {
  ...
  path_info(std::string path_name, key_type path, description_container description)
      : path_name(path_name), path(path), description(description), is_subkey(false), is_sample(false) {}
  path_info(std::string path_name, key_type path, description_container description, description_container subkey_description)
      : path_name(path_name), path(path), description(description), subkey_description(subkey_description), is_subkey(true), is_sample(false) {}

  path_info(const path_info &obj)
      : path_name(obj.path_name), path(obj.path), description(obj.description), is_subkey(obj.is_subkey), is_sample(obj.is_sample) {}

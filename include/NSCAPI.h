...
inline bool api_ok(const NSCAPI::errorReturn &e) { return e == NSCAPI::api_return_codes::isSuccess; }
}  // namespace NSCAPI
...
typedef NSCAPI::nagiosReturn (*lpNSAPIExecCommand)(const char *target, const char *request, const unsigned int request_len, char **response,
                                                   unsigned int *response_len);
typedef NSCAPI::errorReturn (*lpNSAPINotify)(const char *channel, const char *buffer, unsigned int buffer_len, char **result_buffer,
                                             unsigned int *result_buffer_len);
...
// One query entry point, taking a serialised PB::Facts::FactsRequestMessage
// (GET with an optional dotted path, or REFRESH) and returning a
// PB::Facts::FactsResponseMessage (free with NSAPIDestroyBuffer). Read only:
// facts are produced through fetchFacts on a schedule the core drives, so
// there is no push-style counterpart to NSAPISetTag and what each module's own
// configuration enables stays authoritative.
...
typedef NSCAPI::errorReturn (*lpHandleCommand)(unsigned int plugin_id, const char *in_buffer, const unsigned int in_buffer_len, char **out_buffer,
...
typedef NSCAPI::errorReturn (*lpHandleNotification)(unsigned int plugin_id, const char *channel, const char *buffer, unsigned int buffer_len,
                                                    char **result_buffer, unsigned int *result_buffer_len);

typedef NSCAPI::errorReturn (*lpHasRoutingHandler)(unsigned int plugin_id);
typedef NSCAPI::errorReturn (*lpRouteMessage)(unsigned int plugin_id, const char *channel, const char *buffer, unsigned int buffer_len,
                                              char **new_channel_buffer, char **new_buffer, unsigned int *new_buffer_len);

typedef NSCAPI::errorReturn (*lpCommandLineExec)(unsigned int plugin_id, const int targetFlag, const char *in_buffer, const unsigned int in_buffer_len,
                                                 char **out_buffer, unsigned int *out_buffer_len);
...
// Facts producer. The request says why the core is asking this round; the
// response carries the sets the module produced and, per set, why one it
// produces could not be collected. Serialised PB::Facts::FactsQueryMessage and
// PB::Facts::FactsMessage, like every other payload that crosses this ABI.

...
class OLDSettings : public settings::settings_interface_impl {
   ...
   public:
    ...
    void parse_line(std::wstring line) {
      ...
      if (old_key.second == L"*" || old_key.second.empty()) {
        ...
        add(utf8::cvt<std::string>(new_key.first), utf8::cvt<std::string>(new_key.second), utf8::cvt<std::string>(old_key.first),
            utf8::cvt<std::string>(old_key.second));
      }
    }
    std::pair<std::wstring, std::wstring> split_key(std::wstring key) {
      ...
      if (pos == -1) return std::pair<std::wstring, std::wstring>(key, L"");
      return std::pair<std::wstring, std::wstring>(key.substr(0, pos), key.substr(pos + 1));
    }

    void add(std::string path_new, std::string path_old) { sections_.insert(path_map::value_type(path_new, path_old)); }
    ...
    std::string path(std::string path_new) {
      ...
      if (it == sections_.end()) return path_new;
      return (*it).second;
    }
    ...
    std::string status() { return "Sections: " + str::xtos(sections_.size()) + ", " + "Keys: " + str::xtos(keys_.size()); }
    ...
    void get_sections(std::string path, string_list &list) {
      ...
      for (keys_key_type key : keys_) {
        if (path.empty() || path == "/") {
          ...
          if (pos != std::string::npos) key.first.first = key.first.first.substr(0, pos);
 ...
 public:
  OLDSettings(settings::settings_core *core, std::string alias, std::string context)
      : settings::settings_interface_impl(core, alias, context), map(core->get_logger()) {
  ...
  virtual op_string get_real_string(settings_core::key_path_type in_key) {
    ...
    if (has_key_int(in_key.first, in_key.second)) return internal_get_value(in_key.first, in_key.second);
    return op_string();
  }
#define UNLIKELY_STRING L"$$$EMPTY_KEY$$$"
  ...
  std::string internal_get_value(std::string path, std::string key, int bufferSize = 1024) {
    ...
    if (buffer == NULL) throw settings_exception(__FILE__, __LINE__, "Out of memory error!");
    int retVal = GetPrivateProfileString(utf8::cvt<std::wstring>(path).c_str(), utf8::cvt<std::wstring>(key).c_str(), L"", buffer, bufferSize,
                                         utf8::cvt<std::wstring>(get_file_name()).c_str());
  ...
  std::set<std::string> internal_read_keys_from_section(std::string section, unsigned int bufferLength = 1024) {
    ...
    if (buffer == NULL) throw settings_exception(__FILE__, __LINE__, "internal_read_keys_from_section:: Failed to allocate memory for buffer!");
    unsigned int count =
        ::GetPrivateProfileSection(utf8::cvt<std::wstring>(section).c_str(), buffer, bufferLength, utf8::cvt<std::wstring>(get_file_name()).c_str());
  ...
  virtual void set_real_value(settings_core::key_path_type key, conainer value) {
    try {
      key = map.key(key);
      WritePrivateProfileString(utf8::cvt<std::wstring>(key.first).c_str(), utf8::cvt<std::wstring>(key.second).c_str(),
                                utf8::cvt<std::wstring>(value.get_string()).c_str(), utf8::cvt<std::wstring>(get_file_name()).c_str());
    } catch (...) {
  ...
  string_list int_read_sections(unsigned int bufferLength = BUFF_LEN) {
    string_list ret;
    TCHAR *buffer = new TCHAR[bufferLength + 1];
    if (buffer == NULL) throw settings_exception(__FILE__, __LINE__, "getSections:: Failed to allocate memory for buffer!");
    ...
    for (settings_map::section_key_type key : map.sections_) {
      if (key.first == path) {
        ...
        for (const std::string &k : (*it).second) {
          ...
          if (cit == ignore_list.end()) list.push_back(k);
        }
      }
    }
  }
 ...
 private:
  ...
  virtual std::string get_info() { return "INI settings: (" + context_ + ", " + get_file_name() + ")"; }

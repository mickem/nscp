...
| macOS (preview)                | The release's build version and newer             | ARM64 (Apple silicon) | `NSCP-<version>-macos-arm64.pkg`            | Self-contained installer; registers a launchd daemon. Not signed or notarized yet, so install with `sudo installer -pkg ... -target /` rather than double-clicking. No Intel build. Ships without `CheckSystem`, `CheckDisk` and `CheckLogFile`. A relocatable `NSCP-<version>-macos-arm64.tar.gz` is published too, but it needs the Homebrew formulas the build used. |

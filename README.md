...
| Windows (modern)               | Windows 11 ARM, Server 2025 ARM                   | ARM64            | `NSCP-<version>-ARM64.msi`                       | Native ARM64 build, cross-compiled with the `v143` toolset. Use this on Windows-on-ARM devices (e.g. Surface Pro X / Copilot+ PCs, ARM-based Azure VMs). Ships without `PythonScript` (no ARM64 CPython to embed) and without the bundled `check_nsclient.exe` (no ARM64 build published yet); run the x64 package under emulation if you need either. |

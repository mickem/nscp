...
#
# The following global variables are assumed to exist: CSHARP_SOURCE_DIRECTORY -
# path to C# sources CSHARP_BINARY_DIRECTORY - path to place resultant C# binary
# files
#
# The following variables are set: CSHARP_TYPE - the type of the C# compiler
# (eg. ".NET" or "Mono") CSHARP_COMPILER - the path to the C# compiler
# executable (eg. "C:/Windows/Microsoft.NET/Framework/v4.0.30319/csc.exe")
# CSHARP_VERSION - the version number of the C# compiler (eg. "v4.0.30319")
#
# The following macros are defined: CSHARP_ADD_EXECUTABLE( name references
# [files] [output_dir] ) - Define C# executable with the given name
# CSHARP_ADD_LIBRARY( name references [files] [output_dir] ) - Define C# library
# with the given name
#
# Examples: CSHARP_ADD_EXECUTABLE( MyExecutable "" "Program.cs" )
# CSHARP_ADD_EXECUTABLE( MyExecutable "ref1.dll ref2.dll" "Program.cs File1.cs"
# ) CSHARP_ADD_EXECUTABLE( MyExecutable "ref1.dll;ref2.dll"
# "Program.cs;File1.cs" )
#
# This file is based on the work of GDCM:
# http://gdcm.svn.sf.net/viewvc/gdcm/trunk/CMake/UseCSharp.cmake Copyright (c)
...
macro(CSHARP_ADD_PROJECT type name)
  ...
  foreach(it ${ARGN})
    if(${it} MATCHES "(.*)(dll)")
      ...
      elseif(EXISTS ${CSHARP_SOURCE_DIRECTORY}/${it})
        set(SOURCE ${CSHARP_SOURCE_DIRECTORY}/${it})
      elseif(${it} MATCHES "[*]")
        ...
        message(STATUS "Assuming generated file for ${type} ${name}")
        set(SOURCE ${it})
      endif()
      if(SOURCE)
  ...
  message(
    STATUS
      "Adding C# ${type} ${name}: '${CSHARP_COMPILER} /t:${type} /out:${name}.${output} /platform:${CSHARP_PLATFORM} ${CSHARP_SDK} ${refs} ${sources}'"
  )
  add_custom_command(
    COMMENT
    ...
    COMMAND ${CSHARP_COMPILER} ARGS /t:${type} /out:${name}.${output}
            /platform:${CSHARP_PLATFORM} ${CSHARP_SDK} ${refs} ${sources}

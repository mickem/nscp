...
| .NET SDK (8.0+)                        | managed .NET plugin API (`NSCP.Core.dll`), the PowerShell script host (`NSCP.PowerShell.dll`) + C# sample | `dotnet-sdk-8.0`                                 | `modules/dotnet` (the native `DotnetPlugins` module still builds; skip with `-DNSCP_DOTNET=OFF`). The managed build restores `Google.Protobuf` and (for the PowerShell host) `System.Management.Automation` from nuget.org; offline builds point it at a local package folder with `-DNSCP_DOTNET_PACKAGE_SOURCE=<dir>` or skip the restore with `-DNSCP_DOTNET_NO_RESTORE=ON` (packages already in `$NUGET_PACKAGES`), or pass `-DNSCP_DOTNET=OFF`. `-DNSCP_DOTNET_PROTOBUF_VERSION=` picks the runtime version. The C# sample is built into `modules/dotnet` for the tests but is not installed or packaged. The PowerShell package is a compile-time reference only - the engine is loaded from an installed PowerShell 7 at run time and nothing from it is copied into the build. |

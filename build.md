...
| .NET SDK (8.0+)                        | managed .NET plugin API (`NSCP.Core.dll`) + C# sample     | `dotnet-sdk-8.0`                                 | `modules/dotnet` (the native `DotnetPlugins` module still builds; skip with `-DNSCP_DOTNET=OFF`). The managed build restores `Google.Protobuf` from nuget.org, so an offline build with an SDK installed must pass `-DNSCP_DOTNET=OFF`. |

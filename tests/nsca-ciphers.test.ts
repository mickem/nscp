...
dockerOrSkip()("NSCA integration", () => {
  ...
  beforeAll(async () => {
    image = await GenericContainer.fromDockerfile(
      ...
      "Dockerfiles/nsca.Dockerfile",
    ).build("nsca_server", { deleteOnExit: false });
  ...
  it.each(CIPHERS)("submits a CRIT result through encryption=%s (idx %i)", async (cipher, idx) => {
    await submitCipher(cipher, idx);
  });
});

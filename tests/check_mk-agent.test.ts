...
dockerOrSkip()("check_mk integration", () => {
  ...
  beforeAll(async () => {
    ...
    // The check_mk agent dump is generated by `default_check_mk.lua`,
    // which CheckMKServer auto-loads through its own script manager
    // when its [/settings/check_mk/server/scripts] section is empty
    // (see CheckMKServer.cpp:98), so no explicit lua add is needed
    // for the dump itself.
    //
    // We install NRPE + mock.lua only so the Windows shutdown path
    // (see afterAll below) can send `mock_exit` over NRPE — Node's
    // `proc.kill('SIGINT')` is unreliable on Windows for console-less
    // children, and SIGKILL bypasses atexit (hiding leaks from any
    // future sanitizer build). On Linux this is dead weight at boot
    // but harmless; the signal_set in CommandClient handles SIGTERM
    // directly there.
  ...
  afterAll(async () => {
    if (!nscp) return;
    // Windows: send mock_exit over NRPE first so CommandClient sees an

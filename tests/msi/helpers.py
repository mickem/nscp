...
def validate_masked_values(config_folder, file_name, expectations):
    ...
    for section, expected in expectations.items():
        ...
        # No branch below prints a password, its length or any part of it, not
        # even the expected one and not on failure. One of these values is
        # generated by the installer and is a live credential if the hashing
        # ever silently stops working, and this runs in CI, where the log is
        # kept and public. The section name and the test case say the rest.
        if expected == 'absent':
            if actual is not None:
                print(f"! [{section}] has a password and should have none.", flush=True)
                ok = False
            else:
                print(f"- [{section}] has no password, as expected.", flush=True)
            continue
        if actual is None:
            print(f"! [{section}] has no password; the test case expects one.", flush=True)
            ok = False
            ...
            print(f"! [{section}] password is not the value the test case expects.", flush=True)
            ok = False
    return ok

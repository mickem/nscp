...
// Legacy `?password=...` / `?TOKEN=...` query-string authentication was
// removed in commit 340b8db1 for security (credentials leak into browser
// history, proxy logs, and Referer headers). The removal broke Icinga's
// bundled check_nscp_api plugin, which still ships with the query-string
// mechanism. To unblock that integration without re-exposing the vector
// to browsers and arbitrary scrapers, the WEBServer gates the legacy path
// on a User-Agent allowlist. Default: "Icinga/check_nscp_api". Configurable
// via `[/settings/WEB/server] legacy query auth user agents`.
//
// These tests pin both directions:
//   1. A client whose User-Agent matches the default pattern can still use
//      `/auth/token`, `/auth/logout`, and `?TOKEN=...`.
//   2. A client without the matching User-Agent gets the same 410 / 403 the
//      hardening introduced.
const ICINGA_UA = "Icinga/check_nscp_api/2.14.0";

describe("Legacy query-string auth — Icinga allowlist", () => {
    ...
    describe("With matching User-Agent (default allowlist)", () => {
        ...
        it("?TOKEN= query parameter authenticates the issued token", async () => {
            // Sanity-check pre-condition: we got a token from the matching path.

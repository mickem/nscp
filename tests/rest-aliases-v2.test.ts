/**
 * REST alias-endpoint scenarios — migrated from tests/rest/aliases-v2.test.ts.
 *
 * Verifies the QUERY_ALIAS items registered by CheckHelpers under
 * [/settings/check helpers/alias] (see setupRestNscp) show up on /aliases
 * with the expected shape, and that plain queries don't leak into the
 * alias listing.
 */
...
// Aliases are defined under [/settings/check helpers/alias] in the shared
// fixture — the CheckHelpers module registers them as QUERY_ALIAS items at
// load time.
const expectedAliases = ["mock_alias", "echo_alias"];

describe("REST aliases (v2)", () => {
  ...
  describe("v1 alias of the same endpoint", () => {
    let v1Key: string | undefined = undefined;

    it("can login on /api/v1", async () => {
      await request(REST_URL)
        .get("/api/v1/login")
        .auth("admin", "default-password")
        .trustLocalhost(true)

...
class msi_helper {
 ...
 public:
  ...
  ~msi_helper() { hInstall_ = NULL; }

  static std::wstring last_werror(int err = -1) { return utf8::cvt<std::wstring>(error::lookup::last_error(err)); }
 ...
 public:
  ...
  inline bool isNull(MSIHANDLE h) { return h == NULL; }
  ...
  ********************************************************************/
  MSIHANDLE open_execute_view(__in_z LPCWSTR wzSql) {
    MSIHANDLE phView;
    if (!wzSql || !*wzSql) throw installer_exception(L"Invalid arguments!");

    UINT er = ::MsiDatabaseOpenViewW(getActiveDatabase(), wzSql, &phView);
    if (er != ERROR_SUCCESS) throw installer_exception(L"failed to open view on database: " + last_werror(er));
  ...
  ********************************************************************/
  MSIHANDLE fetch_record(__in MSIHANDLE hView) {
    MSIHANDLE phRec;
    if (!hView) throw installer_exception(L"Invalid arguments!");

    UINT er = ::MsiViewFetch(hView, &phRec);
    if (er == ERROR_NO_MORE_ITEMS) return NULL;
    if (er != ERROR_SUCCESS) throw installer_exception(L"failed to return rows from database: " + last_werror(er));
    return phRec;
  }
  ...
  ********************************************************************/
  std::wstring get_record_string(__in MSIHANDLE hRec, __in UINT uiField) {
    if (!hRec) throw installer_exception(L"Invalid arguments!");
...
********************************************************************/
  ...
  ********************************************************************/
  void set_record_string(__in MSIHANDLE hRec, __in UINT uiField, __in_z LPCWSTR wzData) {
    if (!hRec || !wzData) throw installer_exception(L"Invalid arguments!");

    UINT er = ::MsiRecordSetStringW(hRec, uiField, wzData);
    if (er != ERROR_SUCCESS) throw installer_exception(L"Failed to set filed as string!");
  }
  ...
  ********************************************************************/
  std::wstring get_record_formatted_string(__in MSIHANDLE hRec, __in UINT uiField) {
    ...
    if (hRecFormat == NULL) throw installer_exception(L"Record was NULL!");
  ...
  ********************************************************************/
  bool WcaIsInstalling(INSTALLSTATE isInstalled, INSTALLSTATE isAction) {
    return (INSTALLSTATE_LOCAL == isAction || INSTALLSTATE_SOURCE == isAction ||
  ...
  ********************************************************************/
  bool WcaIsReInstalling(INSTALLSTATE isInstalled, INSTALLSTATE isAction) {
    return ((INSTALLSTATE_LOCAL == isAction || INSTALLSTATE_SOURCE == isAction || INSTALLSTATE_DEFAULT == isAction) &&
  ...
  ********************************************************************/
  BOOL WcaIsUninstalling(INSTALLSTATE isInstalled, INSTALLSTATE isAction) {
    ...
    return ((INSTALLSTATE_ABSENT == isAction || INSTALLSTATE_REMOVED == isAction || INSTALLSTATE_UNKNOWN == isAction) &&
            (INSTALLSTATE_LOCAL == isInstalled || INSTALLSTATE_SOURCE == isInstalled));
  }
  ...
  ********************************************************************/
  ...
  class custom_action_data_r {
   ...
   public:
    custom_action_data_r(std::wstring data) : position_(0) {
      ...
      if (pos < data.length()) list_.push_back(data.substr(pos));
    }
    ~custom_action_data_r() {}

    bool has_more() { return position_ < list_.size(); }
    std::wstring get_next_string() {
      if (!has_more()) return L" ";
      return list_[position_++];
    }
    unsigned int get_next_int() { return strEx::stox<unsigned int>(get_next_string()); }
  ...
  class custom_action_data_w {
   ...
   public:
    ...
    void insert_int(int i) { insert_string(strEx::xtos(i)); }
    void write_string(std::wstring str) {
      WCHAR delim[] = {MAGIC_MULTISZ_DELIM, 0};  // magic char followed by NULL terminator
      if (!buf_.empty()) buf_ += delim;
      buf_ += str;
    }
    void write_int(int i) { write_string(strEx::xtos(i)); }
    ...
    bool has_data(const int size = 0) const { return used_size() > size; }
    operator const wchar_t*() const { return buf_.c_str(); }
    std::size_t used_size() const { return buf_.length(); }
  };
  ...
  ********************************************************************/
  HRESULT do_deferred_action(LPCWSTR wzAction, LPCWSTR wzCustomActionData, UINT) {
    ...
    if (wzCustomActionData && *wzCustomActionData) {
      ...
      if (er != ERROR_SUCCESS) throw installer_exception(L"Failed to set CustomActionData for deferred action" + last_werror(er));
    }
    /*
                    if (0 < uiCost) {
